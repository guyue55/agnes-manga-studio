...
- **AI 回原文补人物长相（批 8 补 33）：给模型的输出设计一个"我们验得动"的字段**：
  `char_no_look`（人物卡没有外貌与服装）一直挂 `fixable: false`（"需要人写"）—— 但**原著通常写了**，
  只是抽取那一步漏了。所以定性从"让模型写"改成**"让模型回去找"**，而"找"必须有据 —— 这就是引文。
  （与 `timeline_no_when` 的关键区别：时间点原文没写就是没写、只能猜；长相原文**有据可查**。）
  `POST /api/story/look-fill`：每张候选卡带上**它自己那段原文**，一次调用。
  **核心机制是引文可验证**：模型必须交原文里的**原话**（`quote`），服务端与**那张卡自己**的片段
  （去掉空白后）逐字比对，**对不上就整条丢弃并计 `ungrounded`** —— 这是"抽取"与"编造"之间
  唯一可机械判定的分界线；没有它，编的长相与找到的长相在数据上完全一样，而它会进每一次出图提示词。
  按去空白后比对是有意的：模型常顺手加空格换行，**因格式差异把真找到的丢掉等于把功能废掉一半**。
  四个桶分开报且**互不重叠**：`not_found`（原文没写 —— 诚实的结论，不是失败）、`ungrounded`（它编的）、
  `missing`/`invalid`/`empty`（没给全/编号越界/两项都空）；界面把 `missing+invalid+empty` 汇总成
  "另有 N 张没接住"，所以 `ungrounded` 不能再算进 `missing`（重叠会把同一张卡报两遍）。
  落库过与手改**同一把尺子**（`clipField`）。体检的 `char_no_look` 补上 `go` → 原著页人物卡。
  **顺带查清一条不可达的防御分支**：`no_source` 今天不可达（`evidence`/`chunk_index` 是来源事实、不许手改，
  人物卡只由分块抽取产生、必然带段号；唯一没段号的全局归并卡只产信息卡与剧情卡）——
  apitest 钉的是**这个可达性本身**，将来真加了人工新建卡片的路，那条断言会红。
  **写测试踩的两个坑**（都让钉失去意义）：① "没落库"断言把**对照组**（解析时就有 `白衣` 的那张）算进去了
  → 恒为 1、永远红；② `clearLooks()` 一开始把对照组也清了 → 候选数变了、"已有的一个都不动"这条钉
  在后续用例里**失效**。修法：让对照组**全程**留在候选名单之外。
  **教训**：① **"需要人写"里藏着"其实只要回去找"** —— 同一个 `fixable: false`，能不能自动化的分界是
  **能不能给出可机械核对的依据**；② 凡是让模型补事实类字段，都该先问"**我怎么验证它没编**"，
  答不上来就不该自动写库；③ **"没写成"要分桶，且桶不能重叠** —— 分桶让用户能分辨"原著没写"与"模型在编"，
  不重叠让界面汇总时不会把同一张卡报两遍
...
- 测试：`tools/` 下四套断言脚本（selftest 912 / apitest 1024 / uitest 1128 / browser-test 506），`node tools/run-all.mjs` 全量跑；另有 `node tools/ui-audit.mjs`（真机布局/对比度/截断提示度量报表；4 视口 × 12 页，其中 7 页带**弹窗动作钩子**、2 页带**内联面板动作**，两者都有"声明了动作就必须有产出"的自检，内联钩子用 `box` 指定看哪个容器）与 `node tools/port-check.mjs`（端口撞车防护三场景 6 断言真机验证：含预检环境冲突与收尾无残留自检；exit 0 全过 / 1 违例 / 2 环境冲突），均按需跑、非门禁。
