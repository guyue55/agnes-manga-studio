...
- 测试：`tools/` 下四套断言脚本（selftest 730 / apitest 869 / uitest 1069 / browser-test 497），`node tools/run-all.mjs` 全量跑；另有 `node tools/ui-audit.mjs`（真机布局/对比度/截断提示度量报表；4 视口 × 12 页，其中 7 页带**弹窗动作钩子**、2 页带**内联面板动作**，两者都有"声明了动作就必须有产出"的自检，内联钩子用 `box` 指定看哪个容器）与 `node tools/port-check.mjs`（端口撞车防护三场景 6 断言真机验证：含预检环境冲突与收尾无残留自检；exit 0 全过 / 1 违例 / 2 环境冲突），均按需跑、非门禁。
...
12. **做正对照时不要用 `git checkout <file>` 还原**：它退回的是**上一次提交**，会把这一轮所有未提交的改动
