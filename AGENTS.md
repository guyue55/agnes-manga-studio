...
- **回原文补字段（批 8 补 33 补长相 / 补 34 泛化到场景道具）：给模型的输出设计一个"我们验得动"的字段**：
  `char_no_look`（人物卡没有外貌与服装）与 `no_inject`（地点/道具卡没有任何可注入字段）一直挂 `fixable: false`
  （"需要人写"）—— 但**原著通常写了**，只是抽取那一步漏了。所以定性从"让模型写"改成**"让模型回去找"**，
  而"找"必须有据 —— 这就是引文。（与 `timeline_no_when` 的区别：时间点原文没写就是没写、只能猜；长相/场景**有据可查**。）
  `POST /api/story/field-fill` 用 `target` 选规格（`char_look` / `card_inject`）：先干跑回报"要补几张、调几次"，
  确认后**一次**调用（每张候选卡带上**它自己那段原文**）。**核心机制是引文可验证**：模型必须交原文里的**原话**
  （`quote`），服务端与**那张卡自己**的片段（去掉空白后）逐字比对，**对不上就整条丢弃并计 `ungrounded`** ——
  这是"抽取"与"编造"之间唯一可机械判定的分界线。按去空白后比对是有意的（模型常顺手加空格换行，
  **因格式差异把真找到的丢掉等于把功能废掉一半**）。四个桶分开报且**互不重叠**（重叠会把同一张卡报两遍）：
  `not_found`（原文没写 —— 诚实的结论，不是失败）、`ungrounded`（它编的）、`missing`/`invalid`/`empty`。
  落库过与手改**同一把尺子**（`clipField`）。**两个规格是同一件事**（差别只有"问模型要什么、返回哪个键、卡片怎么称呼"），
  所以抽成 `FILL_SPECS`（两份**数据**）＋**一份**实现 —— **再抄一份的代价不是多写 80 行，而是从此有两份引文核对**。
  `card_inject` 的字段表与 `STORY_INJECT_FIELDS` **同源**（按类别取：地点=氛围/地域/时段/特征，道具=持有者/用途/特征），
  两类**各按自己的上限**截。体检的 `char_no_look`/`no_inject`/`timeline_no_when` 都有 `go` 出口
  （`timeline_no_when` **明说这一项没有 AI 补值**），且**必须落到那张卡上**（`card_id`，有运行时棘轮钉住）。
  **四个"两边各判一半"的坑**（漏带 `kind` → 字段表推成空；路由按 `kind` 重推 → 报 `assigned=2` 却没落库；
  模板变量名对不上 → `renderPrompt` 静默发空；出口只给 `kind` → 几十张卡里还得自己找）**全貌与正对照见
  `docs/research/09-story-bible-plan.md` §6.33/§6.34 与 `docs/issues.md` B95/B96**。
  **教训**：① **"需要人写"里藏着"其实只要回去找"** —— 分界是**能不能给出可机械核对的依据**；凡是让模型补事实类字段，
  都该先问"**我怎么验证它没编**"，答不上来就不该自动写库；② **"没写成"要分桶，且桶不能重叠**；
  ③ 判断要不要泛化的标准是"**机制是否相同**"，不是"代码像不像"；④ **"成功但没写"是最难发现的一类失败** ——
  计划要交代清楚要写什么，执行端照单写，别按别的东西再推一遍；⑤ **同一件事不要有两份来源**
  （正对照 JJ 一开始是**绿的**，删掉冗余立刻 5 红）；
  ⑥ 模板/调用方这种松耦合要在调用前**检查占位符在不在**；⑦ **灵敏度对照本身也会假红** → 改成**确定性反事实**。
- **AI 补分幕次（批 8 补 32）：把"该由模型干的分类"和"不该由模型编的事实"分开**：
  补 31 之后回头数一遍体检里**所有 `fixable: false`** 的项（那就是"必须用户自己动手"的清单）：
  `no_inject`/`char_no_look` 要**写内容**（创作），`plot_no_stage`/`timeline_no_when` 是**分类** ——
  分类正是模型的强项、也最便宜。**先做 `plot_no_stage` 而不做 `timeline_no_when` 是有意的**：
  幕次是**结构标签**（判错顶多分集位置不理想，`planEpisodes` 会如实报），时间点（"三年前"）是**关于世界的事实**，
  模型只能猜，而**猜出来的时间线会当正史进剧本提示词，且与写错的时间线在界面上长得一模一样**。
  幕次是分集的依据（一拍都没标就退化成"按拍数平均切"，`basis: 'count'`），
  `POST /api/story/stage-fill` 先干跑回报"要补几拍、调几次"，确认后**一次**调用（全部拍点一次给完，
  已有的标「已定：X」让模型知道人工边界在哪、补出来才连成一条不倒退的线）。四条纪律：
  ① **只补"待定"的**（模型对"已定"的也给值，一个都不许写）；② **只认四幕词表**，脏值丢弃并报 `not_a_stage`
  ——脏幕次比没有更糟（`stageRank` 当它"没有"，界面**看起来**标了、实际仍退化）；
  ③ **不替模型"修顺"**（它写倒退时只报位置，替它修顺＝我们编数据）；
  ④ 返回 `basis_before`/`basis_after`，把"这次调用有没有用"变成可验收的证据。
  **apitest 抓出一个真错**：`back_steps`（倒退检测）第一版拿**模型的回答**算，可模型对"已定"的拍也给值、
  **那些值我们根本不写** —— 于是报了一次"没落库的倒退"＝假警报。改成按**落库后的真实状态**算，
  并且**成对钉**（该报的 + 不该报的），否则把检测整个关掉也是绿的。
  体检的 `plot_no_stage` 同时补上**出口**（`go` 直达原著页 `panel=outline`，那里就是补幕次的按钮），
  但**不**改成 `fixable: true`：补幕次要花钱，混进"免费的一键修复"会让用户在不知情时花钱。
  **教训**：① **"需要人动手"的清单要真的数一遍** —— 判断类的每一项都是"用 AI 减少人工"的候选，
  此前被一句"需要人拍板"整体跳过了；② **补结构标签 ≠ 补事实**，涉及世界设定的字段先问"它凭什么知道"；
  ③ **报"异常"要按"最终真的会怎样"算，不是按"模型说了什么"**，拿被丢弃的提议去报异常就是假警报
...
- 测试：`tools/` 下四套断言脚本（selftest 978 / apitest 1075 / uitest 1141 / browser-test 507），`node tools/run-all.mjs` 全量跑；另有 `node tools/ui-audit.mjs`（真机布局/对比度/截断提示度量报表；4 视口 × 12 页，其中 7 页带**弹窗动作钩子**、2 页带**内联面板动作**，两者都有"声明了动作就必须有产出"的自检，内联钩子用 `box` 指定看哪个容器）与 `node tools/port-check.mjs`（端口撞车防护三场景 6 断言真机验证：含预检环境冲突与收尾无残留自检；exit 0 全过 / 1 违例 / 2 环境冲突），均按需跑、非门禁。
