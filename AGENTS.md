...
- **提示词"能更新" + 剧情卡人物闭环（批 8 补 28）**：① **提示词是抽取质量最大的杠杆，而它到不了老用户手里** ——
  ...
  （行上 `builtin_digest` 自证，或内容指纹在历史表 `TEMPLATE_SUPERSEDED` 里）；**证明不了就当成用户改过、原样保留**
  （覆盖人的劳动不可逆），启动横幅如实报"更新了几个 / 保留了哪几个"（纯函数 `planTemplateSync`，六条路径 selftest 钉）。
  **内容已等于官方默认的行要顺手补记指纹**（`stamped`），否则它永远拿不到指纹、将来真改这些提示词时会被当成
  "用户改过"而**永不更新**。**反向红线**：绝不给 `edited`/`custom` 补记 —— 那等于把用户内容登记成官方版，
  下次同步必覆盖它（**把静默不更新变成静默丢数据**）。**会写用户数据的启动副作用必须放在"确认自己是唯一实例"之后**
  （播种在 `listening` 回调里，同 `poller.resume()`）—— 放模块顶层时端口被占而退出的**影子实例也会写库**，
  而 `persist` 是整库快照，影子写的更新会被在跑的实例下一次快照**静默覆盖**。
...
- **回原文补字段（批 8 补 33/34/35：补长相 → 场景道具 → 时间点，加上"引文可见"）**：`char_no_look`/`no_inject`
  ...
  **`timeline_when` 是补 32"有意排除"的反转**（"只能猜"已被引文核对拆掉：猜的没有引文可交）；
  残余风险是"引文真、却**认错是哪一句**"，**只有人能看出来** → 配**引文可见**：报告面板逐张列出
  "**写进去的值 ← 它的原文原话**"，值取**实际落库**那一份（截断之后），`assigned_names` 与 `assigned_items` **同源**，
  且报告**留在页面上**（toast 会消失，回头就核对不了）。体检三项都有 `go` 出口且**必须带 `card_id`**（运行时棘轮钉住）。
  **教训**：① **"需要人写"里藏着"其实只要回去找"**，分界是**能不能给出可机械核对的依据** —— 凡让模型补事实类字段，
  先问"**我怎么验证它没编**"，答不上来就不该自动写库；② **"没写成"要分桶且不重叠**；③ 要不要泛化看"**机制是否相同**"；
  ④ **"成功但没写"最难发现**：计划交代写什么、执行端照单写；⑤ **同一件事不要有两份来源**；
  ⑥ 模板/调用方这种松耦合，调用前**检查占位符在不在**；⑦ **一条被排除的理由会在上游条件变化后失效** ——
  反转时断言与文案一起改（别删），并把反转的**前提**写清楚；⑧ **能机械判定的交给代码，剩下的必须让人看得见**；
  ⑨ **报告要报"实际落库的那一份"**。四个"两边各判一半"的坑见 §6.33–§6.35 与 `docs/issues.md` B95–B97。
...
- **逐集生成也要看见「全剧设定」（批 8 补 36）：同一个事实有两条路，就一定会分叉**：`episodeOutlineText`
  ...
  **不同的上下文**，且各自看都正常；② **字段闭环要对两个方向对差集**（要了没存 / 存了没人读）；
  ...
  （对照 KT 把设定从 brief 里拿掉，那条弱断言照样绿）；④ 新断言先兜底再取字段：KV 第一次跑**整轮零输出**，
  不是没红，而是 `p.setting_text.includes(...)` 在 `undefined` 上抛 `TypeError` **中止了整轮**（注意事项 14 复发）。
...
- **角色名册进输入指纹（批 8 补 37）：喂给模型的输入，指纹里也必须有**：补 6 让生成剧本/分镜时把角色名册
  ...
  静默 bug 最快的方法（补 36 找到设定、补 37 找到名册，**同一张清单剩下的应该一次数完**）；跨端点的同源
...
- **AI 补分幕次（批 8 补 32）：把"该由模型干的分类"和"不该由模型编的事实"分开**：补 31 之后回头数一遍体检里
  **所有 `fixable: false`** 的项（那就是"必须用户自己动手"的清单）：`no_inject`/`char_no_look` 要**写内容**（创作），
  ...
  在哪、补出来才连成一条不倒退的线）。四条纪律：① **只补"待定"的**（模型对"已定"的也给值，一个都不许写）；
  ...
  **出口**（`go` 直达原著页 `panel=outline`），但**不**改成 `fixable: true`：补幕次要花钱，混进"免费的一键修复"
  ...
  的候选，此前被一句"需要人拍板"整体跳过了；② **补结构标签 ≠ 补事实**，涉及世界设定的字段先问"它凭什么知道"；
  ③ **报"异常"要按"最终真的会怎样"算，不是按"模型说了什么"** —— 第一版 `back_steps` 拿模型的回答算，
  可模型对"已定"的拍也给值、**那些值我们根本不写**，于是报了一次"没落库的倒退"＝假警报；
  改成按落库后的真实状态算，并且**成对钉**（该报的 + 不该报的），否则把检测整个关掉也是绿的。
...
- **角色字段的唯一漏斗（批 8 补 31）：同一个实体有 7 个写入点，正确答案不是"每处都改对"**：
  ...
  **原文**注入**每一次出图提示词**，而 `characterFields` 只有 `str().trim()`，**任何路径都没有上限**
  ...
  问题** —— 2 条路能靠"两边都记得"维持，**7 条不行**，正解是让"绕过去"在结构上不可能；② **上限要算不要拍**，
  "更严 = 更安全"是错的，**小于合法输入就是静默丢数据**；③ **文本字段的归一化不能顺手用在非文本字段上** ——
  一个 `str()` 能把 `is_locked` 变 `"false"`（非空字符串＝真值，用户没勾选却被锁定）、把 `reference_image_ids`
  变字符串（出图再也带不上参考图），而且都不报错；漏斗里"只动文本字段、其余原样带过"要**显式写出来**并有断言。
...
- 卡片溯源（批 8 补 20）：卡片上只有"证据段 3"这种**段号**，想核对就得自己回原文数段 → 要么盲信、要么重读整本。
  ...
  **来源事实**、白名单外改不动；删原著**级联删卡片**（否则留下"指不到原文的孤儿卡"）
...
- 参考图缺口体检（批 8 补 19）：补 13/补 17 把参考图接进了出图调用，但"接上了"不等于"有人挂" ——
  ...
  按钮 —— 体检不能只有结论、没有出口；跨页参数名要对得上（原著页 `project_id`/`source_id`，角色库页 `project`）。
...
- 地点卡/道具卡参考图进出图输入（批 8 补 17）：人物卡有资产库，地点/道具卡却只有一行文字 → **同一个场景每张图
  ...
  重新解析/追加解析就会把用户挂的图**静默清空**（selftest 钉的是"形状"而不是"值"，对照 BG）
...
- 测试：`tools/` 下四套断言脚本（selftest 1076 / apitest 1187 / uitest 1181 / browser-test 538），`node tools/run-all.mjs` 全量跑；另有 `node tools/ui-audit.mjs`（真机布局/对比度/截断提示度量报表；4 视口 × 12 页，弹窗与内联面板两类动作钩子都有"声明了动作就必须有产出"的自检）与 `node tools/port-check.mjs`（端口撞车防护三场景 6 断言真机验证；exit 0 全过 / 1 违例 / 2 环境冲突），均按需跑、非门禁。
...
- **推荐路线**：① 拷走 `.trash-*` 的 `tmp/` 与 `batches.json`；② 按批（一轮一批，别一次全做）拿 `gen-batch-<N>.mjs`
  ...
  ⑥ 用 SKILL.md 的内联校验脚本（`tmp/ua-inline-validate.cjs`）验完，再写 `knowledge-graph.json` + `meta.json` +
  `fingerprints.json`，⑦ 连同代码改动一起提交。
