...
- **提示词"能更新" + 剧情卡人物闭环（批 8 补 28）**：① **提示词是抽取质量最大的杠杆，而它到不了老用户手里** ——
  ...
  **内容已等于官方默认的行要顺手补记指纹**（`stamped`），否则将来真改这些提示词时会被当成"用户改过"而**永不更新**；
  **反向红线**：绝不给 `edited`/`custom` 补记（那等于把用户内容登记成官方版，**把静默不更新变成静默丢数据**）。**会写用户数据的启动副作用必须放在"确认自己是唯一实例"之后**
...
- **逐集生成也要看见「全剧设定」（批 8 补 36）：同一个事实有两条路，就一定会分叉**：`episodeOutlineText`
  ...
  **不同的上下文**，且各自看都正常；② **字段闭环要对两个方向对差集**（要了没存 / 存了没人读）；
  ...
  不是没红，而是取 `undefined` 上的字段抛 `TypeError` **中止了整轮**（注意事项 14 复发）。
...
- **分镜输入指纹漏了名册（批 8 补 38）：把"喂给模型的输入"一次数完**：补 37 的教训写着"同一张清单剩下的
  ...
  **前端一行不用改**（同一份正文 + 同一个函数必然同源）。精确性同补 36/37（没角色逐字节不变、
  ...
  清单上还剩两条：图片/视频提示词（原地写回，"派生字段过期"）与剧本润色（另存一条）。
...
- **角色字段的唯一漏斗（批 8 补 31）：同一个实体有 7 个写入点，正确答案不是"每处都改对"**：
  ...
  **小于合法输入就是静默丢数据**；③ **文本字段的归一化不能顺手用在非文本字段上** —— 一个 `str()` 能把
  `is_locked` 变 `"false"`（非空字符串＝真值，用户没勾选却被锁定）、把 `reference_image_ids` 变字符串
  （出图再也带不上参考图），都不报错；漏斗里"只动文本字段、其余原样带过"要**显式写出来**并有断言。
...
- 测试：`tools/` 下四套断言脚本（selftest 1093 / apitest 1200 / uitest 1185 / browser-test 541），`node tools/run-all.mjs` 全量跑；另有 `node tools/ui-audit.mjs`（真机布局/对比度/截断提示度量报表；4 视口 × 12 页，弹窗与内联面板两类动作钩子都有"声明了动作就必须有产出"的自检）与 `node tools/port-check.mjs`（端口撞车防护三场景 6 断言真机验证；exit 0 全过 / 1 违例 / 2 环境冲突），均按需跑、非门禁。
