## B99 角色名册喂给了模型，却没进输入指纹（批 8 补 37）

**症状**：往角色库里**新增一个角色**、或改了某个角色的**外貌**之后，已经生成的剧本**一个字都不会变**，
界面上的「过期体检」照样显示 `ok`。可是模型下次生成时会看到一份**不同的角色名册**（补 6 把名册写进请求体，
用来对齐"女主/苏婉儿/婉儿"混着写的名字）—— 也就是说：**输入已经变了，指纹没变，产物静默地与输入不符**。

**根因**：名册**只在前端算**（`public/js/consts.js` 的 `characterRoster`），而"输入变了要报过期"靠的是
**服务端**算的输入指纹（补 12 的 `episodeInputDigest`，当时只覆盖本集拍表 + 前情）。于是名册是
**"喂给了模型、却不在指纹里"**的那一份输入 —— 与 B98（设定）是同一类病，只是换了一个字段。

**为什么不能"顺手在服务端再写一份"**：名册的渲染规则（排序 / 上限 30 / 别名的两种形态 / 长相截 30 字）
写在前端，服务端再写一遍就等于**同一套规则两处实现** —— 正是补 30/31/B98 反复钉死的病根，
而它在这里的表现是"**提示词里的名册与指纹里的名册不是同一份**"（两边各自看都正常）。

**修法**：
① 渲染收敛到服务端 `story.characterRoster` **唯一一份**，前端那份**删掉**（搬家，不是复制）；
② 两个生成入口（故事脚本页 / 分镜页）改走同一个新端点 `POST /api/story/roster`
   （**纯计算、不写任何东西**；用 POST 是因为要渲染的文本动辄几千字，塞进 query 会撞 414）；
③ 名册**挂到分集结果上**（`planWithRosters` 产出 `plan.roster_texts`）而不是加一个参数 ——
   指纹有**两个调用点**（`episode-brief` 存 / `staleness` 复算），漏挂一处那一边就会算出另一个指纹，
   后果是**每一集永久报过期**；合成一步，"漏挂"在结构上变成不可能；
④ 载入某一集时前端**采纳服务端那份**名册（与 `input_digest` 同源），不再现算一遍把它盖掉；
⑤ 载入提示如实报出"名册 N 个角色"（带没带在界面上长得一样就白带了）；
⑥ 排序改用显式**码点比较**（`byName`）：名册顺序进指纹，`localeCompare` 依赖运行环境 locale/ICU，
   会让同一份输入在不同机器上算出不同指纹（补 24 已为"多文件排序"定过同一条纪律）。

**精确性（两条）**：
· **没有角色时指纹逐字节等同于从前**（不 push 空串）—— "项目里一个角色都没有"的用户不该凭空多出一批过期。
  apitest 用一个自证的闭环证明：角色从无到有 → 指纹变；再从有到无 → 指纹**逐字节回到最初那一份**
  （说明变化的来源只有名册，没有别的东西在漂）。
· **只改 `personality`/`role` 不许报过期**（它们不进名册），并配**前提断言**证明那次 PUT 真的写进去了 ——
  否则反向钉是空的（注意事项 11）。

**教训**：
① 把"**喂给模型的输入**"逐条列一遍，是找这类静默 bug 最快的方法 —— B98 找到设定、B99 找到名册，
**同一张清单还剩几条应该一次数完**，别一轮只治一个；
② **跨端点的同源不能靠注释**：两处各算一次的东西，要让"漏挂一处"在**结构上不可能**（合成一个构造函数），
并配一条**会红的**反事实断言（拿不带名册的 plan 去判定，明明没改也会报 `stale`）；
③ **搬家优于复制**：前端那份删掉、两个入口共用一个端点，比"在服务端再写一份再钉同构"更彻底；
④ **会进指纹的东西不许有环境依赖**（locale / 时间 / 随机 / 遍历顺序）；
⑤ 钉"不许用某个 API"的棘轮要**剥掉注释再查** —— 注释里为了说明"不许用"往往会写出那个 API 名，
直接 `!includes` 会被自己的注释判红（同时要正向要求替代实现真的在，否则"函数不存在"也能绿）。

## B98 逐集生成看不见「全剧设定」，而全剧大纲那条路一直带着它（批 8 补 36）

**症状**：同一部戏里，用「带入剧本」（一次性，走全剧大纲）写出来的剧本**知道**这部戏的题材/基调/世界观/主线，
而用「逐集生成」（长篇真正会走的那条路）写出来的**完全不知道** —— 写第 7 集时模型眼里只有本集拍表 + 前情，
于是各集的口气、主线推进、世界观细节各写各的。**两条路的产物在界面上长得一模一样**，没有任何报错。

**根因**：设定块原先只在 `episodeOutlineText` 里拼（它的注释还专门写了"刻意把全剧设定/全剧时间线一并带上：
否则跨集的设定漂移就是这么来的"），而 `episodeBriefText` 只渲染本集拍表。**同一个事实有两份来源，
其中一份只在一半的路上存在**。这不是"忘了写"，是**结构上没有单一来源**。

**顺带抓到的四个孤儿字段**：修这一轮时做了补 29 的**反向**对差集（"存了却没人读"），
立刻发现 `CARD_FIELDS.world` 的 6 个字段里，`cardLine` 只渲染 `题材`/`基调` ——
`世界观`/`主题`/`一句话简介`/`主线`**抽出来了、卡片界面上也显示，却到不了任何提示词**。
补 29 治的是"要了却没存"，这里是"存了却没人读"。

**修法**：
① 抽 `settingLines(cards)` 作**唯一一份**渲染（`'【全剧设定】'` 全项目只许出现一次，uitest 棘轮钉死）；
② 把它挂到**分集结果**上（`planEpisodes` 返回 `setting_text`）—— 调用方**想忘也忘不掉**；
③ 全剧大纲与单集拍表都从 `plan.setting_text` 取，`episodeInputDigest` 复用 `episodeBriefText`
   所以设定**自动**进指纹，不需要第二处同步代码；
④ 信息卡的 6 个字段全部进 `cardLine`；
⑤ 端点回传 `setting_chars`/`world_count`/`timeline_count`，故事脚本页据此如实说出"含全剧设定 N 张"。

**精确性（本轮最要紧的取舍）**：**不加"指纹版本号"**。加了会让"这份原著根本没有信息卡"的用户
凭空多出一批过期（假警报比不检查更糟）；现在没有设定卡时 `setting_text` 是空串、
**指纹与从前逐字节一致**。反向也钉了：改 `world.summary`（不进上下文的字段）**不许**报过期，
并配一条前提断言（`summary` 确实写进去了），否则那条钉可能是空的。

**教训**：
① **"同一个东西有两条路"要像"有两份真相"一样认真对待** —— 后果不是显示不一致，
而是同一条链上两个产物拿到**不同的上下文**，且各自看都正常；
② **字段闭环要对两个方向对差集**（要了没存 / 存了没人读），只做一个方向会漏掉一半；
③ **精确性靠"不改就一个字节都不变"** —— 不引入版本号、不"顺手多算一点"，
否则体检从"可信"变成"常年挂着"，用户学会无视它；
④ **报出去的数必须与真的发出去的那一份同源**：弱断言（`setting_chars > 0` 且 `< brief.length`）
挡不住"数与文本对不上"（正对照 KT 把设定从 brief 里拿掉，那条弱断言照样绿）；
⑤ 写新断言前先问"把它关掉会红吗"，并**先兜底再取字段** —— KV 第一次跑整轮零输出，
不是没红，而是我的新断言 `p.setting_text.includes(...)` 在 `undefined` 上抛 `TypeError` **中止了整轮**
（注意事项 14 的老毛病）。

## B97 `setBusy(container, …)` 把整页内容与所有监听一起抹掉，而链路上零报错（批 8 补 35）

**症状**：点「AI 补长相」/「AI 补场景字段」/「AI 补幕次」后，toast 如实报出"已补 N 张"，
看起来功能正常 —— 但**卡片列表从来没有刷新**（界面还显示旧描述）、分集大纲没有重切、
而且**整页从此变哑**（工具栏、卡片、所有按钮点下去都没反应）。控制台**一句错都没有**。

**根因**：`setBusy(el, true, label)` 的语义是"把**按钮**的 `innerHTML` 换成 spinner"。
补 32「AI 补幕次」与补 33/34「回原文补字段」都写成了 `setBusy(container, true, …)`，
而 `container` 是 router 挂进文档的**页面容器**。于是：
① `container.innerHTML = '<span class="spinner">…'` → **整页 DOM 被换成一行 spinner**；
② 紧接着的 `loadCards()` / `runOutline()` / `renderFillReport()` 全都在**spinner 里**找节点 →
   `container.querySelector('#nov-cards')` 返回 `null` → `null.innerHTML = …` 抛 `TypeError`；
③ 这些错被 `finally` 里的 `setBusy(container, false)` **掩盖**：它把 busy 前存的
   `_origHtml`（一段**字符串**）贴回去，于是节点是**新的**、里面所有 `onclick`/`addEventListener`
   **全部丢失** —— 页面看起来恢复了，其实已经变哑。
净效果：**"成功但没写"的最坏形态** —— 花钱了、报成功了、用户以为做完了，而数据没更新、页面不能用。

**为什么一直没抓到**：这两条路此前**只有文本断言**（uitest 看源码字符串、`node --check` 看语法），
没有任何**真机**测试点过它们。本轮给"回原文补字段"补了真机契约，**第一次跑就抓到**（面板一直不出现、
box 始终是 `<div id="nov-fill-box"></div>`）。

**修法（三道，缺一不可）**：
① **调用点传按钮**：`fillFields(target, btn)` / `fillStages(btn)`，由 `e.currentTarget` 传入。
② **`setBusy` 自身拒绝非控件**：`tagName` 不在 `BUTTON|INPUT|A` 里就**直接返回**，
   并把这次误用推进 `window.__uiRejects`（真机测试的全局钩子会看到）。
   注意这条防呆让"误用"降级成"spinner 不显示"而不是"页面被毁"——
   正对照 JQ 里**面板断言反而是绿的**，红的只有那条"不许留下未处理拒绝"。
③ **uitest 结构棘轮**：页面模块里不许出现 `setBusy(container`。

**正对照 JR 的第一版钉法是绿的（"钉存在"≠"钉生效"）**：uitest 最初只断言
`/setBusy 只能传按钮/` 这个字符串在 `ui.js` 里 —— 把 `if` 条件换成 `if (false)`，
**字符串还在**、断言照样绿。改成"条件必须**紧挨着**拒绝逻辑"（按索引切片后匹配），
并在 browser-test 加**真机行为**断言：动态 `import('/js/ui.js')` → 喂一个 `div` →
页面内容必须原样留着 + 误用必须进 `__uiRejects`；再配一条"**真按钮**照常进加载态"
（否则"一律拒绝"也能让前两条通过）。改完重跑同一次注入：uitest 1 红 + browser-test 3 红。

**教训**：
① **"共享小工具"的参数语义必须被结构性地保证**：`setBusy` 的文档注释写着"按钮元素"，
   但类型是 `HTMLElement`，传容器进去语法完全合法 —— 于是三个人（补 32/33/34）都传错过。
   把"合法但错"变成"运行时拒绝 + 测试可见"，比在注释里写清楚有效得多。
② **真机测试的价值不在"更全"，在于它是唯一能看见"页面状态"的那一层**：
   源码字符串全对、语法全对、接口全对，而页面上什么都没有 —— 只有真机点击能发现。
   一条功能只要**用户会点**，就该有一条真机契约（这两条路此前恰好没有）。
③ **`finally` 里的"还原"可能把错误藏起来**：`setBusy(container,false)` 把 HTML 贴回去，
   于是异常不再向上冒、页面"看起来"恢复了 —— 排查时表现为"点了没反应"，而不是"报错了"。
   凡是 `finally` 里做的都是**补救动作**，补救动作本身不该掩盖失败（本轮靠 `__uiRejects` 才看见）。
④ **注意事项 11 第三次复发**：`/renderStaleBar\(h\)/` 匹配到函数定义、补 25 的"黑衣/白衣都是 2 字"、
   本轮的 `/setBusy 只能传按钮/` 匹配到死代码 —— 都是"钉了存在、没钉生效"。
   最省事的判别法：**把它关掉，这条断言会红吗？** 不会红就说明钉错了地方。

---

## B96 补 33 的机制被抄第二遍就会漂；还有三个"成功但没写"级别的坑（批 8 补 34）

**症状**：体检里 `no_inject`（地点/道具卡没有任何可注入字段）一直只有结论、没有出口，
用户只能自己一张一张点开填。而 `STORY_INJECT_FIELDS` 里的字段是**会进画面描述**的承诺
（`storyCardPhrase`）—— 不补，同一个场景在 20 个镜头里长得都不一样，而每张图单独看都"成功"。

**根因①（同一机制的第二份实现）**：`no_inject` 与补 33 的 `char_no_look` 处境完全一样 ——
原著写了那个场景/道具，只是抽取漏了，同样"回原文找"是**抽取**而不是创作、同样能用**引文**验证。
差别只有"问模型要什么、返回哪个键、卡片怎么称呼"。**再抄一份的代价不是多写 80 行，
而是从此有两份引文核对**：改一处、另一处不红，直到某天发现"地点卡的引文检查怎么不生效"。

**根因②（出处条目漏带 `kind`）**：`applyFillAssignments` 拿到的是 `fillExcerpts` 产出的**条目**（不是卡片），
而"该补哪些字段"是按类别推的 —— 条目没有 `kind`，字段表就是空的，
于是**每一条都落进 `empty`、一个字都没写**。症状还会骗人：接口报 `assigned=0`、`empty=N`，
看着像"模型没给全"。发现它靠的是**把落库结果打出来**（`patch: []` 而 `offered: 1`）。

**根因③（路由按 `kind` 重推字段表）**：写入循环写的是 `fillFieldsOf(spec, p)`，
而 `p` 是**计划条目**（同样没有 `kind`）→ 字段表又空了 → **接口报 `assigned=2`，卡片上却一个字都没有**。
这比 ② 更坏：状态码对、计数对、提示对，**只有去看卡片才知道没写**。

**根因④（模板变量名对不上是静默失败）**：泛化时路由统一传 `卡片与原文片段`，
而 `char_look` 的模板里还写着 `{{人物与原文片段}}` —— `renderPrompt` 会把"调用方没给的 `{{…}}`"
替换成**空串**，于是**卡片清单整个发不出去**，模型只能凭名字编。链路上**零报错**，
表现是"顺利路径三条断言同时红，但完全看不出为什么"。

**修法**：
① 抽成 `FILL_SPECS`（`char_look`/`card_inject` 两份**数据**）＋**一份**核对实现，
`POST /api/story/field-fill` 用 `target` 选规格；`card_inject` 的字段表与 `STORY_INJECT_FIELDS` **同源**（按类别取）。
② 出处条目带上 `kind`。
③ **计划自己交代要写哪几个字段**（`fields`），调用方照单写；并且**把没人读的 `kind` 从计划里删掉** ——
   正对照 JJ 一开始是绿的（因为冗余的 `kind` 让"按 kind 重推"又能工作了），删掉它立刻变 5 红。
④ 规格里记 `varName`，**路由调用前先检查模板里有这个占位符**，没有就明确报错。
⑤ 给 `no_inject` 与 `timeline_no_when` 补出口，且**出口必须落到那张卡上**（`card_id`）；
   补的时候发现补 33 的 `char_no_look` **也漏了**，于是加**运行时棘轮**：
   对真实体检结果断言"凡是按类别落到某张卡的出口，都必须带 `card_id`"。
⑥ 文案不为了共用而变含糊：`emptyNote`/`noneNote` 放进规格（"都已经有外貌或服装了"比"都填好了"有用）。

**顺带修掉一条会假红的灵敏度对照**：防连点那条"等 400ms 再点第二次"依赖"第一次请求还没回来、弹窗还开着"，
机器快的时候弹窗已关、第二次点的是**摘下来的节点**（等于没点）→ 报"对照不敏感"。
改成**确定性**反事实：直接连发两次 `POST /api/projects`，证明**接口本身不去重**，
于是"界面上只产生 1 个"确实来自防连点。**对照的价值在于反事实成立，不在于动作像不像真人。**

**计数**：selftest 912 → **978**，apitest 1024 → **1075**，uitest 1128 → **1141**，browser-test 506 → **507**，全绿。
**正向对照（7 条，全部精确命中，还原后字节一致）**：JG 关掉引文核对 → selftest 6 红；
JH 判据丢掉"类别"那一半 → selftest 6 红（四条既有断言立刻报出"地点卡缺长相"这种假警报）；
JI 去掉变量守卫 → apitest 2 红；JJ 路由按 kind 重推 → apitest 5 红（assigned=2 却没落库）；
JK 条目不带 kind → selftest 6 红；JL 出口不带 card_id → selftest 3 红；JM 多给的键照写 → selftest 1 红。

## B95 人物的长相被当成"要人写"，其实是"回原文找"（批 8 补 33）

**症状**：体检一直把"人物卡没有外貌与服装"报成 `char_no_look` / `fixable: false`（"需要人写"），
用户只能自己一张一张点开填。而 `appearance`/`outfit` 是**会原文进每一次出图提示词**的字段
（`characterPhrase`）—— 不补，同一个角色在不同镜头里就是不同的脸。

**根因（把"抽取"错当成"创作"）**：补 32 数 `fixable: false` 清单时，把 `char_no_look` 与 `no_inject`
一并归成"创作、风险高"。但**原著通常写了**这个人的长相，只是抽取那一步漏了 ——
那么"回原文再找一遍"是**抽取**，不是创作。与 `timeline_no_when` 的关键区别：
时间点原文没写就是没写、只能猜；长相原文**有据可查**。

**修法**：`POST /api/story/look-fill` —— 每张候选卡带上**它自己那段原文**，一次调用。
**核心是引文可验证**：模型必须交原文里的**原话**（`quote`），服务端与那张卡自己的片段
（去掉空白后）逐字比对，**对不上就整条丢弃并计 `ungrounded`**。这是"抽取"与"编造"之间
唯一可机械判定的分界线 —— 没有它，编的长相与找到的长相在数据上完全一样，而它会进每一次出图提示词。
四条纪律：① `found:false`（原文没写）计 `not_found`，是**诚实的结论**不是失败，绝不拿编的顶上；
② 引文必须来自**这张卡自己**的片段（模型把 A 的引文安到 B 头上最容易发生也最难发现）；
③ "原文没写 / 引文对不上 / 没给全"**三个桶分开报**，且 `missing` 与 `ungrounded` 不重叠
（界面把 `missing+invalid+empty` 汇总成"另有 N 张没接住"，重叠就会把同一张卡报两遍）；
④ 落库过与手改**同一把尺子**（`clipField`）。体检的 `char_no_look` 补上 `go` → 原著页人物卡。

**查清的一条不可达分支**：`no_source`（这张卡没有可定位的原文出处）今天**不可达** ——
`evidence`/`chunk_index` 是来源事实、不许手改，而人物卡只由分块抽取产生、必然带段号；
唯一没有段号的全局归并卡只产信息卡与剧情卡、不产人物卡。apitest 钉的是**这个可达性本身**，
将来真加了人工新建卡片的路，那条断言会红。

**写测试时踩的两个坑（都让钉失去意义）**：① "没落库"断言用
`filter(c => c.appearance || c.outfit).length === 0`，可对照组林晚解析时就有 `白衣` → 断言**恒为 1、永远红**；
② `clearLooks()` 一开始把 4 张全清（包括对照组），候选数从 3 变 4，"已有的一个都不动"这条钉
在后续用例里就**失效**了。修法：让对照组**全程**留在候选名单之外。

**验收**：selftest 钉纯函数语义（只挑缺的、出处只取这张卡自己的段、别名也算命中、超长截断、
`has_text` 与 `found:false` 与 `ungrounded` 的语义、引文跨卡不算数、越界/重复编号、三个桶不重叠、
判据与体检同源）、模板（回原文找 / 交原话 / 逐字比对 / 找不到就说 / 不许写性格画风）、体检出口；
apitest 端到端（干跑不写 → 补上且已有的不动 → 引文对不上原文一个字都不落库 → 原文没写计 not_found →
超长过同一把尺子 → 不可解析算失败且不动卡片 → 没有候选时 calls 0 → 可达性 → 体检出口 → 404）；
uitest 钉前端接线（按钮、先干跑再 `costConfirm`、"要补几张"来自服务端、三种原因分开报、
切块不一致要提醒、补完重载）。
**正对照**：IX（不核对引文）= selftest 4 + apitest 8 红；IY（`found:false` 也写）= 2 + 2 红；
IZ（引文跨卡算数）= selftest 2 红；JA（不干跑）= uitest 1 红；JB（去掉体检出口）= 3 + 3 红；
JC（不过长度尺子）= apitest 1 红。

---

## B94 幕次一直要人手工标，而它只是分类（批 8 补 32）

**症状**：`planEpisodes` 靠**幕次**（起/承/转/合）在幕的收口处切集；一拍都没标就退化成"按拍数平均切"
（`basis: 'count'`，切点可能落在一幕中间）。体检一直把它报成 `plot_no_stage`，但 `fixable: false`、**也没有 `go`**
—— 用户看到"30 张剧情卡没有标幕次"，只能自己一张一张点开填。而"给 30 个拍点标幕次"是**纯分类**。

**根因（两层的"没想到"）**：
① 体检把所有"需要人拍板"的项**整体**归成 `fixable: false` 就结束了，从没再问一句"这里面哪几件其实**只是判断**"——
   判断正是模型的强项，也是最便宜的调用（一次调用、几个字）；
② 这一项连**出口**都没有（`plot_no_stage` 没有 `go`）—— 典型的"只有结论、没有出口"，
   与 `plot_cast_unknown`/参考图缺口那两项的待遇不一致。

**修法**：`POST /api/story/stage-fill` —— 干跑回报"要补几拍、调几次"，确认后**一次**调用补齐
（全部拍点一次给完；已有的标「已定：X」，让模型知道人工划的边界在哪）。
四条纪律：① 只补"待定"的（模型对"已定"的也给值，一个都不许写）；② 只认四幕词表，
脏值丢弃并报 `not_a_stage`；③ 不替模型"修顺"（写倒退时只报位置）；④ 返回 `basis_before`/`basis_after`
作为"这次调用有没有用"的证据。体检的 `plot_no_stage` 补上 `go` → 原著页 `panel=outline`。

**为什么**不**顺手把 `timeline_no_when` 也做了**：幕次是**结构标签**（判错顶多分集位置不理想），
时间点（"三年前"、"第二日清晨"）是**关于世界的事实** —— 模型只能猜，而猜出来的时间线会**当作正史**
进剧本提示词，且**猜的和写错的在界面上长得一模一样**。补结构标签和补事实是两件事。

**apitest 抓出的真错（假警报）**：`back_steps`（"幕次倒退"检测）第一版拿**模型的回答**算，
可模型对"已定"的拍也给了值，而**那些值我们根本不写** —— 于是报了一次"根本没落库的倒退"。
改成按**落库后的真实状态**算。并且**成对钉**：既钉"不该报的别报"，也钉"该报的必须报"
（只钉前者的话，把检测整个关掉也是绿的）。

**验收**：selftest 钉纯函数语义（只补待定/词表过滤/越界与重复 index/`no_id`/真倒退要报/`count`→`stage`）、
模板与 `STAGE_ORDER` 同源、体检出口；apitest 端到端（干跑不写库 → 补上 → 人定的没被动 → 脏值丢弃 →
不可解析算失败且不动卡片 → 真倒退照写并报位置 → 没有待定时 `calls: 0` → 体检出口）；uitest 钉前端接线
（按钮只在有"待定"时才渲染、先干跑再 `costConfirm`、结果如实上报、深链开面板）。
**正对照**：IR（不过滤词表）= apitest + selftest 红；IS（覆盖"已定"的）= selftest 红；
IT（`back_steps` 改回按模型的回答算）= selftest 红；IU（去掉体检出口）= selftest + apitest 红；
IV（原著页不认深链）= uitest 红；IW（跳过计费确认）= uitest 红。

---

## B93 角色的 7 个写入点，谁都没管长度（批 8 补 31）

**症状**：角色档案里的「外貌描述」可以粘任意长，保存也成功 —— 而它会被 `characterPhrase`
**原文**注入**每一次出图提示词**（角色锁定时）。实测：POST 一个 20000 字的 `appearance`，**存回 20000 字**，
之后每一次出图都带着这 20000 字。用户以为自己只是"写得详细一点"。

**根因（两层）**：
① `characterFields` 只做 `str().trim()`，**任何路径都没有上限**（人物卡那边是 200）；
② 更根本的是 `characters` 有 **7 个写入点**（建/改/单卡导入/批量导入/体检修复·入资产库/体检修复·锁定/`sync_character`），
它们**各自 clip 了一部分字段**，数字还不一样 —— `notes` 有 **400（cardToCharacter）/ 400（sync 写死）/ 500** 三个数，
`alias` 在"人物卡导入"与"体检同步"上**完全没截**（而别名会随角色名册进每一次 LLM 请求）。

**修法（关键：不是"每处都补一句 clip"）**：在每处补 clip 只是**把病根再抄一遍**（补 30 已经证明手抄清单会漂）。
现在只留两个出口 `insertCharacter` / `updateCharacter`，尺子在函数体里，**任何写入都绕不过去**；
字段清单（`CHARACTER_EDITABLE_FIELDS`）与上限表（`CHARACTER_FIELD_MAX`）都在 `story.js`。
与人物卡**同名**的字段直接取 `FIELD_MAX` 的同一个数（"人物卡同步到资产库"本来就用 `clip(card[k], k)`，
另定一套就会出现"卡里存得下、资产库被砍"）；角色独有的 `alias`/`notes` 单列。

**数字是算出来的，不是拍的**：`alias` 一开始定 120 看着宽松 —— 算一下才发现**会截合法数据**：
卡片别名最多 6 个、每个 ≤20 字（`normalizeAliases`），拼起来 6×20+5 = **125**；
而体检同步会把库里已有别名与卡片别名**取并集**，两套满编约 **250**。改成 300 才装得下。
**"更严 = 更安全"是错的：小于合法输入就是静默丢数据。**

**漏斗自身最容易被弄坏的地方**：`clipCharacterPatch` 只处理**文本**字段、其余原样带过。
`is_locked` 一旦被 `str()` 成 `"false"`（非空字符串）就是**真值** —— 用户没勾"外貌锁定"，外貌却被锁定；
`reference_image_ids` 一旦变成字符串，出图时就再也带不上参考图。两条都不报错。apitest 专门钉了它们。

**验收**：四条钉 —— 同名同尺（selftest）、**漏斗不可绕过**（uitest 结构棘轮）、前后端上限表同构（uitest）、
端到端截断 + 能清空 + 非文本字段不被字符串化（apitest）。
**正对照**：IM（漏斗不过尺子）= apitest 8 红；IN（绕过漏斗直写）= uitest 2 红；
IO（前端上限表漂一格）= uitest 1 红；IP（同名另定一套尺子）= selftest 3 红；IQ（卡片上限表漂一格）= uitest 1 红。
