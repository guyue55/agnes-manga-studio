## B110 管进程的脚本最容易错的两件事都不报错：照着过期 pid 文件 kill、假设端口就是 5178（**已实施**）

**背景**：项目一直是手工 `node server.js` 起服务，改完后端代码要记得重启（不重启的表现是"接口不存在"，
像功能没做）。现在补一套 `scripts/`：`start / restart / stop / status / build`，后台常驻、日志与 pid 落
**数据目录**下（`data/run/`）。

**两条纪律（都来自 server.js 已有的机制，不是新发明）**：
① **能证明是我们的进程，才动手**。pid 文件会过期、PID 会被系统复用 —— 照着它 `kill` 可能杀掉无关进程。
   判据与 server.js 的 `sameAppAlive` **同源**：`/api/health` 的 `ok=true` 且 `data_home` 就是本仓库数据目录；
   命令行里出现本仓库 `server.js` 的**绝对路径**只作兜底。证明不了就**拒绝动手**并说清怎么人工核对。
② **端口是问出来的，不是假设的**。server.js 在端口被**别的**程序占用时会自动 +1 重试（最多 40 次），
   所以"我让它监听 5178"不等于"它就在 5178"；脚本把**实际**端口探出来写进 pid 文件。
   另外 **"进程退出 0"≠"启动成功"**：同一数据目录已在跑时 server.js 会打印"已在运行"再 `exit 0`
   （X4 防护，本身是对的），所以启动成功与否看**健康体里的 pid 是不是我这次拉起的那个**。

**写的时候真踩到的三个坑（都不是"语法错误"，语法检查全过）**：
① **BSD sed 的 BRE 不支持 `\|` 分支**（那是 GNU 扩展）。`h_bool` 写成 BRE 后**静默返回空串**，
   而空串被当成"否" —— 于是**在运行的服务被判成"没在运行"**，`status.sh` 报"○ 没在运行"而服务就在 5178 上。
   改成 `sed -E`（ERE）后正常。**教训**：取值器返回空串时必须当"不知道"处理，且要有一条真机断言钉住它。
② **bash 3.2（macOS 自带）会把变量名后面那个多字节字的首字节吃进变量名**：`"$x（…"` 报
   `x\xef: unbound variable`。21 处变量紧跟全角标点，全部改成 `${x}`。**教训**：脚本要跟自带 bash 对齐，
   别拿"我这台机器的 bash 是 5.x"当前提。
③ **运行期状态一开始放在仓库 `build/run/`** —— 而本工具要用**临时数据目录**跑测试，于是它会**覆盖线上实例的
   pid 文件**（差点自己踩到）。改成放**数据目录**下：判据统一成"同一份数据 = 同一个实例"，两份数据目录各有各的 pid 与日志。
   顺带确认 `lib/store.js` 用的是显式路径（`db.json` / `assets/` / `exports/`），不扫描目录，加子目录是安全的。

**验收**：`tools/scripts-check.mjs`（22 断言，真起进程/真发信号/真探端口，用临时数据目录 + 高位端口，
不碰线上实例）—— 覆盖 pid/端口/健康体三者一致、不重复启动、restart 换 pid 且旧进程真没了、
**端口被外来程序占用时报出实际端口**、**过期 pid 文件指向活着的无关进程时拒绝动手且不误杀**、
stop 幂等与收尾无残留。按需跑、非门禁（要空闲端口，环境冲突时 exit 2 而不是报成产品违例）。

## B109 UI 与创作流程"关联不强"的根因不是缺功能，而是**同一件事有 5 份口径**（UI 重构 B5.1–B5.5，**已实施**）

**用户原话**："现在的 UI 和创作流程关联不强，整个项目、功能、UI、能力都很分散，可以参考学习该项目的设计，
优化重构本项目 UI"（参考 `/Users/apple/Project/Git/Webeye-Video`，只借设计决策、不抄代码）。
B1–B4 治的是**设计语言 / 体验缺陷 / 组件质感**，B5 治的是**骨架** —— 所以先把参考项目的 IA 读成一份研读报告
（`docs/research/ui-ia-flow-spine.md`：四层 IA、侧栏即进度、`stage` 是持久化字段、镜头级 4 步清单）。

**症状 A（同一个事实有 5 种口径）**：项目上下文此前由**8 个页面各写一套**"当前项目是哪个"的解析 ——
`params.project || params.project_id || state.projects[0] || localStorage[...]` 这类链条在 6 个创作页里
各抄一遍，取值顺序与键名还**互不相同**（原著页认 `project_id`、角色库页认 `project`、图片页还额外认
`agnes.assets.project`）。表现是：① "这一页选好项目、切到另一页又变回第一个"；② **同一个链接在不同页面上
指向不同项目** —— 这不是"分散"，这是**同一个事实有 5 份互相矛盾的答案**。
**修法**：壳层持有 `state.projectId`，全站唯一入口 `resolveProjectId(params)` / `rememberProject(id)`；
项目参数的**规范名统一成 `project`**，旧别名 `project_id` 只在 `parseHash` 一处读入、由 `syncViewParams`
一次性迁走（旧链接不失效）；换项目**全站只剩侧栏一个控件**，页头改成只读标签（`projectLabel`）。
素材库那个下拉**故意保留** —— 它是**跨项目筛选器**，不是"当前项目"，两件事不能合并。

**症状 B（我第一版的错：拿快照当权威）**：`resolveProjectId` 一开始用 `state.projects` 判"这个项目存不存在"，
而它是**启动那一刻**的列表 —— 于是**刚建好的项目被判成死链**、页面静默切到别的项目（真机 `分镜毒数据可见` 超时抓到）。
**教训**：**快照不是权威**。URL 是用户的意思，先照它渲染；"是不是真的没了"要拿**一份新数据**异步核对，
且三种结局必须分开：① 在 → 什么都不说，顺手刷新快照；② 真的没了 → 说明并切走；③ **核对不了 → 什么都不说**
（把"问不到"说成"不存在"就是假警报）。
**同一个坑的第二形态**：死链提示写好了却**从不出现** —— 因为守卫写成 `if (state.projectId !== id) return;`，
而壳层在一次 SSE 触发的 `refreshState` 里**自愈**了过期的 `projectId`，于是守卫先一步返回。
修法：判"用户是不是已经走了"要**看地址栏**（`parseHash().params.project`），不看内存状态。

**症状 C（导航不承担"我卡在哪"）**：七段链（原著 → 卡片 → 分集骨架 → 剧本 → 分镜 → 图 → 视频）
此前只在**工作台**看得见 —— 而用户最需要知道"下一步点哪儿"的时候，恰恰是他**已经在某一页干活**的时候。
**修法**：链抽成 `public/js/pipeline.js`（**唯一一份**实现：紧凑流程条 / 详细面板 / 侧栏徽标 / 页头那一行四处共用），
`#pipe-strip` 挂在 `#view` **之外**（页面重挂不会把它清掉），侧栏每个入口带"这一步欠着什么"的徽标。
服务端每一步补 `nav`（属于哪个侧栏入口）与 `unit`（数字单位）—— `nav` 与 `page` 是**两个事实**
（分镜图/分镜视频在分镜页做，但账算在图片/视频入口上），各留一个字段，别让"去哪儿做"兼职"算哪个入口的账"。

**症状 D（三处各写一份语气表 = 三个数字会分叉）**：工作台面板原本自己写着 `BADGE`/`WORD` 两张表。
**教训**：三处各写一份的代价**不是多几十行，而是数字会分叉，而分叉没人看得出来**。

**真机抓到的自己的 bug（两个）**：
① **徽标取错了那一端**：`pipelineBadge` 原本取"**最靠后**那个没做完的"步 —— 原著入口认领"原著/卡片/分集"三步，
   原著落库之后，最靠后的没做完的是 `blocked` 的分集 → 徽标显示"待前置"，**把可行动的"该做了解析卡片"藏了起来**
   （后面的步没做，往往**正是因为前面那步没做**）。改成取**最靠前**那个（与 `next_step` 同一判据）。
② **页头与流程条各说各的**：页头写"下一步：分集剧本（待前置）"，流程条同时写"下一步：解析出卡片" ——
   两处各说各的，用户没法判断该信哪个。**根因**是把"这一步是什么"与"**可行动的**下一步"当成了同一个问题：
   被上游挡住时，这一页的"下一步"其实**在上游**。修法：`blocked` 的步页头**不摆"下一步"**，
   只如实说"上游产物还没做完"。

**教训汇总**：① 快照不是权威，**URL 才是用户的意思**；② 同一个事实只留**一个控件 / 一份口径**；
③ "可行动的下一步"与"这一步是什么"是两个问题；④ **TTL 挡的是高频重复，不是换项目/换页**
（换页强制刷，否则流程条显示上一页的旧数字）；⑤ 钉"两个状态画得不一样"要钉**判据**（既不同色又不同词），
**别钉某段标记的形状**（形状一改，钉子就变成测空气）；⑥ 页面不许手写"上游是谁" —— 那是第二份口径。

**B5.6（项目级"上次停在哪一段"）**：项目卡上给一个"继续创作 · <上次那一段>"，点整张卡同落点。
这一轮真正花时间的地方不是按钮，而是**"哪些页在链上"这个判据从哪儿来** —— 三个候选里只有一个是干净的：
① **手抄一份页面清单**（`['novel','scripts',...]`）：加/删链上步骤时会**静默分叉**，而且分叉表现为
"某个入口永远没有徽标"或"继续创作把人带去一个不在链上的页"，两处都看不出来；② **每次现查该项目的内存缓存**
（`progressOf`）：缓存是**按项目**的，刚换项目时那份可能还在路上 —— 于是"这一页在不在链上"被答成"不在"，
**第一次访问某个项目永远记不住位置**（偶发、只在真机换项目时出现，正是最难查的一类）；③ 用**服务端**每次返回的
`steps[].nav` 去重集合（`isChainNav`）—— 采用这个。首次拉取在 boot 里是**被 await 的**，所以任何页面挂载时
这个集合都已经有了，判据既不依赖时序也不依赖手抄。
**记录时机也是坑**：`state.projectId` 是**页面**在挂载时（`resolveProjectId`）解析出来的，
在 `await nav.page(...)` **之前**记，会把"当前页"记到**上一个项目**头上 —— 于是"继续创作"永远差一格。
**与 `next_step` 的关系**：两件事，不许合并 —— "我上次在哪"只有本机记得住，"这个项目下一步该做什么"
是七段链的事实（属于流程条）。所以按钮标签写的是"上次在…"、**不冒充下一步**；没有记忆时回落到分镜页
（把空字符串交给 `navigate` 会白屏）。入口中文名取自侧栏那张表（`navLabel`），不在项目卡里另写一张。

**B5.11（分镜表每行自带"下一步"）**：参考项目"三个可直接搬的设计决策"里的第三条（每一行自带下一步），
此前只做到**页级**（5.5 页头写"上游产物 + 下一步"），行级一直只有三个产出点 + 出图/出视频按钮 ——
用户看得出"缺什么"，但得自己把三个点翻译成"我该点哪个"。现在每行在三个点下面写一行
"下一步：补提示词／出分镜图／出视频"。
**这一条的价值不在那行字，而在"同源"**：三道关（提示词→分镜图→视频）的推导原本内联在 `outputCell` 里，
如果"下一步"另写一份判断，两处迟早分叉，而分叉的样子是"点还灰着、文案却说下一步是别的关"——
**两处各自看都对，只有并排看才看得出**（B5 教训③的老病）。所以先把它收敛成 `rowSteps(s)` **一份**，
产出点与"下一步"都读它，"第一个没做完的关"就是下一步（与侧栏徽标同一个判据）。
三条边界：① 三道关都齐时**不硬凑**下一步（写"三道关都齐了"—— 假的下一步比没有更糟）；
② **在跑/失败要说清是哪一关**（写"下一步：出分镜图"会让用户以为该再点一次，其实正在跑）；
③ 顺手把一条**钉形状的旧钉子**改成钉判据：原来那条断言钉的是 `dot(promptState, promptTitle)` 这类
**变量名**，重构一次就变成测空气 —— 现在钉"三个点由 `rowSteps` 一份推导"。
**正对照**：把"下一步"固定说成"出视频"、`rowNext` 不动 → **uitest 全绿**（它钉的是推导），
而**真机那两条红得干净**（`{"txt":"下一步：出视频","i":0,"states":["pending","idle","ok"]}`）——
一处失败把"点说是提示词、文案说是视频"原样摆出来。这正是分工：文本层钉推导，真机层验同源。

**测试**：selftest 1302 / apitest 1395 / uitest 1322 / browser-test 637，全绿。跨文件棘轮三条：
服务端标的 `nav` 必须是**真实存在**的侧栏入口（拼错一个字母 = 那个入口永远没有徽标，
而"没有徽标"看起来跟"这一步没做"一样）；五个创作页的"我是哪个入口"必须都报；
`pipeline.js` 里**不许出现手抄的链页面清单**。**正对照两条（都做了）**：① 删掉 `rememberStage` 里
"在不在链上"的把关 → 断言红；② 去掉项目卡的回落 → 两条断言红。**第①条第一次做对照时没红** ——
因为那条钉子对整个 `pipeline.js` 匹配 `isChainNav(navId)`，命中的是**定义**行
（`export function isChainNav(navId)`），把关删掉它照样在。**注意事项 11 第四次复发**：
写"调用了 X"的断言必须限定在**调用点所在的函数体**里找；判别法只有一句 ——
**把它关掉，这条断言会红吗？**真机契约（browser-test ⑧）：先停在**设置页**（不在链上）——
断言它**没有**被记成"上次停下的位置"，再看项目卡是不是"继续创作 · 原著解析"、点下去是否真的回到 `#/novel`。

## B108 补 32 那份"只能人工"的清单**数漏了一整族**；补上最后一项 AI 出口时又差点留下一条永久假警报（批 8 补 45，**已实施**）

**症状 A（数漏了）**：补 32 立下的方法论是"体检里所有 `fixable: false` 的项 = 必须用户自己动手的清单"。
可那份清单是**顺着 `auditCards` 的码表数出来的** —— 而体检的 `issues` 是**四个函数**合起来的：
`auditCards`、`auditShotBindings`、`auditRefImageGaps`、`auditCharacterDrift`。顺着一个函数的码表数，
就漏掉了另一个函数报出的**一整族**：`ref_image_missing` / `ref_image_local_only` / `ref_image_dangling`
（参考图缺口，`lib/story.js` 的 `auditRefImageGaps`，`base` 里写死 `fixable: false`、`counts.fixable: 0`）。
**教训**：数"清单"之前先数**"清单是从哪儿来的"** —— 判据有几处来源，"数完了"这句话就要有几处出处。

**症状 B（那一族接不上 AI）**：参考图缺口的唯一出口是"挂一张**公网**图片 URL"。看起来"AI 生成一张参考图"
是顺理成章的出口，**但它是假修复**：本项目**没有上传能力**（`lib/agnes.js` 只调 5 个 Agnes 端点，
无 upload/files/assets，`body.image` 只接受公网 URL），`isPublicUrl` 把本地 `/assets/…` 与 `data:` 一律排除。
于是"生成 + 挂上"只会得到 `ref_image_local_only` —— **看起来做完了，实际一张都不会生效**。
更要命的是反向：若把生成结果的**远端 URL** 挂上去，体检会判"配好了"（`resolveRefs` 只判"有没有可用的公网 URL"），
而远端 URL 会过期 → **体检绿着、上游 404**，纯本地体检**判不了"那个 URL 现在还能不能打开"**。
这条边界必须写下来，否则将来有人会以为"体检绿了就万事大吉"。
（调研结论：Agnes 网关**没有**上传端点、也没有 base64/data URI 支持；参考包里 5 个第三方项目**全部不用 Agnes**，
它们各自靠自家后端上传 —— 而这要求服务公网可达，本地 127.0.0.1 部署下不可验证，正是本项目 R26 早已决定不立项的理由。）

**修法（这一轮真正做的）**：把清单上**最后一项 AI 确实做得了**的接上 —— `shot_char_unknown`
（镜头「出场人物」里角色库找不到的名字）。落点是**两处一起写**：
① `character_ids` 绑定（**并集**语义，与补 5 的自动匹配同一个落点）；② 把「出场人物」里的**代称换成本名**。

**症状 C（我第一版就是这么错的）**：第一版只写①不写②，理由是"不改用户写的字更安全"。结果是：
体检的 `shot_char_unknown` 判的是**那段文本**，只绑不改 → 这条体检**永远消不掉**，
而它宣称的后果（这些镜头不会有外貌注入）**其实已经修好了** —— 这就是**假警报**，比不检查更糟。
更糟的是用户会再点一次：候选照旧在 → **真的再花一次模型调用** → 绑定已存在 → 报"模型没有认出任何一个"，
**而它明明认出来了**。三件事同时错：假警报、白花钱、报告说谎。
**教训**：修一条体检问题之前，先问"**它判的是哪一个事实**" —— 只修好它宣称的**后果**、不修它判的**那个事实**，
问题不会消失，只会变成假警报。两处写的本来就是同一个事实（这个人是谁），**必须一起写**。

**红线与分桶**（与补 33–35/43 同一套）：`target` 必须真的在角色名册里才绑（名册外如实报 `unknown_target`，
既不静默忽略、也不硬套一个最像的角色 —— 绑错了会注入**别人的脸**）；三个结论桶**互不重叠且都不是失败**：
认出来（`bindings`）/ 是路人（`passerby`）/ 原著里有、项目里没档案（`need_card`，出口是补人物卡）；
引文对不上镜头文字 → `ungrounded`（那是它编的）。**候选只来自体检自己报出来的那几条** ——
与用户眼睛看到的列表是同一份，于是"体检报 3 个、认名字只认 2 个"在结构上不可能（补 43 的教训）。

**出口挂在问题上**：`shot_char_unknown` 原来**既没有 `fixable`、也没有 `go`** —— 报出来了、却一个按钮都没有，
用户只能读详情文字自己找去哪一页。现在体检项带 `ai_fix: {code, label}`，面板把按钮渲染在**那一行**里；
`fixable` 仍为 `false`（认名字要花钱，混进"免费的一键修复"会让用户在不知情时花钱，补 32 立的规矩）。

**已知残余**：`passerby` / `need_card` 这两类结论**不会**让体检那条消失（名字确实不在角色库，体检没报错），
所以用户下次还会看到它、再点一次还会花一次调用。要做到"AI 看过了、结论是路人"不再重复报，
需要在分镜上记一条"已认过"的状态 —— 那是**新增数据 + 新增失效模式**，这一轮按"能不动就不动"**不做**，在此登记。

## B107 「回原文找」有五件事、五个按钮、五次计费确认 —— 用户得自己记住"还差哪一步"（批 8 补 44，**已实施**）

**症状**：补 32–35/43 把体检里"只能人工"的项一项一项交给 AI，于是原著页工具栏上并排出现了五个付费按钮：
`AI 补幕次` / `AI 补人物卡` / `AI 补长相` / `AI 补场景字段` / `AI 补时间点`。
**机制完全一样**（干跑闸门 → 计费确认 → 一次调用 → 引文核对 → 分桶上报），但用户要：
自己看体检报告判断哪几项有候选 → 依次找到那五个按钮 → 每一个都点一遍干跑 → **每一个都过一次计费确认** →
最后在五个互相覆盖的报告之间来回对照。而且**顺序有讲究**（补人物卡要排在补长相前面：新建的卡也可能缺长相），
这件事此前只写在两个按钮的提示里，没人会照做 —— 顺序错了就得多点一轮、多花一次钱。

**修法**：工具栏加一个「AI 一键补齐」，把这一族**一次做完，只弹一次计费确认**。
关键在它比"依次点五遍"**更安全**而不是更危险，靠三件事：

1. **先免费问一遍**。五类的干跑都是**纯本地**的（一次模型都不调），先把"各有多少候选、共调几次"如实摆出来再确认。
   钱是**准的**（每一类固定 1 次调用），会随步骤漂的只有"各有多少候选"—— 确认框里**明说这一点**，
   否则用户看到报告里的数与刚才不一致会以为出了错（"调用次数不变"这句话是给这个用的）。
2. **每一步开跑前重新干跑一次**（干跑不要钱）。前面的步骤会**改数据**：新建的人物卡也可能缺长相，
   于是后面的候选会变多。拿最开始那份名单直接开跑，就是对着**过期名单**花钱。
3. **失败只丢这一步**。某一步报错不影响其余步骤，报告里那一步单独说"这一步没做成、为什么"，
   并提示"可以单独再点一次那一类的按钮重试"。

**一个真判据、一个假警报**（这一轮最容易做错的地方）：`card_inject` 这类端点在"这份原著还没有这类卡片"时
返回 **HTTP 400** —— 它是**结论**（这件事现在无从谈起），不是故障。一键补齐如果把它报成"这一步没做成"，
用户就会去找一个**不存在的问题**，而"假警报比不检查更糟"。所以分类**看状态码、不猜文案**：
`status === 400` → `skipped`（把服务端原话当理由摆出来）；其余（5xx/网络）→ `failed`。
能这么分是因为这几个端点的 400 **只有一个含义**，而"模板里少了变量"这类真故障走的是 500
（补 43 加的变量守卫是 `throw new Error`，不是 `httpError(400, …)`）—— 这条区分本身也是不变量。

**报告只有一份正文实现**：`fillReportBody(d, cfg)` 同时给"单个补"与"一键补齐"用，
批量报告只是把每一步的那一段按顺序拼起来。另写一份的代价不是多几十行，而是**同一个事实的第二份形状** ——
分叉的表现是"报告里少了一列"，没人会发现（补 43 刚在服务端踩过同一个坑）。

**步骤表里一个判据都没有**：`FILL_ALL_STEPS` 只写"调哪个端点、叫什么"，"有几张缺"全部来自服务端干跑。
前端再抄一份判据迟早会漂（补 30 的教训）—— 这一条有 uitest 结构棘轮钉着。

### 实施记录（补 44 已完成）

- `public/js/pages/novel.js`：新增 `FILL_ALL_STEPS`（五步，顺序有意）、`STAGE_FILL`（幕次那一类的"叫什么"）、
  `fillAll`（编排：干跑 → 一次确认 → 逐步重干跑 → 跑 → 合并报告）、`renderBatchReport`；
  把原 `renderFillReport` 的正文抽成 `fillReportBody`（**单个补与批量共用**）、外壳抽成 `mountReport`。
  工具栏加「AI 一键补齐」按钮。**没有新增端点** —— 五步调的是与逐个按钮**同一个**端点，
  于是"有几张缺"不可能出现第二份判据（这是本轮唯一重要的架构决定）。
- 测试：uitest **1232/0**（+10）、browser-test **592/0**（+14，新增真机组：只弹一次确认 /
  某一步 500 失败其余照常落库 / 400 被当结论不是失败 / 再点一次只对还有候选的那类收钱 /
  报告"值 ← 原话" / 页面真的刷新 / 无未处理拒绝）。selftest 与 apitest **未变**（本轮无服务端改动）。
- 正对照 6 条（PA–PF）全部红得干净、逐字节还原：PA 去掉"每步重干跑"、PB 把 400 也当失败、
  PC 失败时抛出中断整批、PD 批量报告另写一份正文、PE 先确认再干跑、PF 步骤表里自算判据。
  **PB 值得记一笔**：它对着 uitest 那条"`dry.status === 400`"的**形状钉**照样绿（我把条件改成
  `false && …`，字符串还在）—— 那条钉只保证"这段代码没被删掉"，真正的行为判据在 browser-test 里
  （"有 1 步没做成"），PB 打过去红了 2 条，红出来的正是那句**本该避免的假警报**。
...
- **病 ①（漏更新，且静默）**：官方这一轮只改了某个模板的 `system`（正文没动）→ 两边的 content 指纹相等
  → 直接判成 `same` → **不更新**，而且 `keep` 里连一条记录都没有（启动横幅只说"更新了 0 个"）。
  实测（真函数）：`update: 0 | keep reason: same`。这与 B102/B103 尾部记的那个缺口是同一句话，
  只是它在**同步侧**。要命的是**代码里第 26 行的维护约定早就写着"改了 content/system 就登记旧指纹"** ——
  承诺写在注释里，实现只兑现了一半（补 25 的"写进注释的承诺必须由代码兑现"第 N 次复发）。
- **病 ②（覆盖人的劳动）**：用户只在设置页改了 `system`（`#tf-sys` 是真实存在的输入框，
  `PUT /api/templates/:id` 也确实收 `system`），正文没动 → 行上仍留着我们发的 content 指纹
  → 一旦官方这一轮改了**正文**，`ours` 成立 → `update` 会**把 content 与 system 一起写**
  → **用户自己写的 system 被静默抹掉**（不可逆）。实测：`update: 1`，写入两个字段。

**病根**：`builtin_digest` 的语义是"这一行是不是我们发出去的那一版"，但它只覆盖了模板的**一半**。
补 41 刚在**输入指纹**那一侧把这句话钉死（`templateDigest` 覆盖 `system + content`，
"只哈希 content 就是半个指纹"），而**同步**这一侧还是半个 —— **同一个概念有两份来源**，
而且更弱的那一份负责的正是"会不会覆盖用户数据"。

**修法（已设计完，待实施）**：让两侧**共用同一份** `story.templateDigest`，并把判据**逐字段**化 ——
因为"能证明 content 是我们的"与"能证明 system 是我们的"是**两个不同的证明强度**：

| 证明 | 含义 | 可以写哪些字段 |
| --- | --- | --- |
| 全量证明 `row.builtin_digest === templateDigest(cur)` 或 `templateDigest(cur) ∈ 历史表` | 整行就是我们发的 | content + system |
| 正文证明 `row.builtin_digest === digestText(cur.content)` 或 `digestText(cur.content) ∈ 历史表` | **只有正文**是我们发的（老库的行上就是这种指纹） | **只能写 content** |

判定顺序（六条路，纯函数）：
1. `templateDigest(cur) === templateDigest(def)` → `keep:'same'`（**顺手补记全量指纹** —— 见迁移）；
2. `cur.is_builtin === false` → `keep:'custom'`；
3. 全量证明成立 → `update`（写 content + system，与今天同）；
4. 只有正文证明成立、且 `cur.content !== def.content` → `update_content`（**只写 content**，
   指纹改写成本次的正文指纹，`system` 原样保留并点名上报）；
5. 只有正文证明成立、且 `cur.content === def.content`（官方只改了 system）→ `keep:'system_kept'`
   （不写任何字段，**但点名上报**"官方改了 system，而这一行的 system 证明不了是官方版，已保留"）；
6. 正文也证明不了 → `keep:'edited'`（今天的语义，不变）。

**迁移（关键，做错就变成更糟的故障）**：把 `same`/`ours` 从"正文指纹"换成"全量指纹"之后，
老库里行上的**正文指纹**就不再等于新算出来的全量指纹 → 会被判成 `edited` → **从此再也不更新**
（把"漏一次 system 改进"换成"什么都收不到"）。所以：
- 老指纹必须**继续被接受**（上表的"正文证明"那一行就是为此存在的）；
- 内容与 system **都**等于官方默认的行（`reason==='same'`）要**顺手补记全量指纹** ——
  只写 `builtin_digest`/`builtin_version`，不动任何内容。这是**安全的**（这一行本来就与我们逐字相同），
  而且它把"从今往后"接上：下一轮官方再只改 `system` 时，行上的全量指纹就能自证 → 更新真的会发生。
- **反向红线不变**：`system_kept`/`update_content` 这两种"只证明了一半"的行**绝不补记全量指纹** ——
  那等于把用户写的 system 登记成官方版（补 28 立下的红线）。

**诚实的边界**：本轮之前装的库里，若某行的 `system` 已被用户改过、而我们这一轮又恰好只改了该模板的
`system`，这一行**仍然证明不了**（老指纹只覆盖正文，旧 system 没有留档，两种可能长得一模一样）
→ 按既定纪律选"保留"（漏一次改进 < 覆盖人的劳动）。区别是**它不再静默**：`system_kept` 会点名上报。
而迁移之后（行上有了全量指纹）这类缺口不会再出现。要不要把"历届官方 system"也留档以彻底消掉这个残余，
单独评估 —— **不要为了消灭一个已被点名的残余去赌覆盖用户数据**。

**为什么这轮只记不做**：这是**唯一一处会在启动时动用户已有数据**的逻辑，而上面每一步都有一条
"做错就变成更糟的故障"的分支（迁移会退化成"永不更新"、逐字段会退化成"覆盖用户 system"）。
按"能不动就不动 / 不要引入新故障"，先把证据与设计留档，下一轮带着正对照一次做完。

### 实施记录（补 42 已完成）

- `lib/seed.js`：`planTemplateSync` 判据改成**逐字段两档证明**（全量 `templateDigest` → 可写 `content + system`；
  旧式正文指纹 → **只能写 `content`**），新增 `update_content` 桶与 `system_kept` 保留理由；
  `seedTemplates` 的 `update_content` 分支**只写正文**、指纹登记成"正文那一半"；
  `stamped` 循环改登记**全量**指纹（＝迁移）；`server.js` 横幅新增「提示词提醒」行把 `system_kept` **点名上报**。
- 写这一版时**真踩到一个自己造的 bug**：`digText(cur)` 传了**整行**而不是 `cur.content`
  （哈希的是 `"[object Object]"`）→ 四条断言一起红，说明"用真内容做 fixture"是这次能立刻发现它的原因。
- 正对照 5 条（全部红得干净、逐字节还原）：**NE** `same` 退回只比正文（2 红）／**NF** `update_content` 顺手写 `system`（1 红）／
  **NG** 给 `system_kept` 也补记全量指纹（1 红，反向红线）／**NH** 补记退回正文指纹（2 红，迁移失效）／
  **NI** 正文证明被当成全量证明（8 红）。**NI 第一次跑让 selftest 以 `TypeError` 中止整轮**
  （我新加的"点名上报"断言直接取 `keep[0].def`，注意事项 14 第 N 次复发）→ 改成 `(kept(...).def || {}).key` 后红得干净。
- 真机验收：重启前 20/20 行带**旧式正文指纹**；重启后 20/20 变成**全量指纹**，而 `content` 变化 0 行、
  `system` 变化 0 行 —— 迁移只动指纹，一个字节的内容都没改（这一轮之后单独改 `system` 才收得到）。

## B104 生成模板也是输入：改了设置页的剧本模板，旧剧本照旧报"没过期"（批 8 补 41）

**症状**：在设置页把「分集剧本」模板改一段（加一句要求、换一套口吻），
故事脚本页里**已经生成的剧本一个字都不变**，过期体检对它们仍然报 `ok`。
更普遍的一层：`story_concept` / `plot_summary` / `episode_outline` / `storyboard_script`
这**四个非分集页签**的剧本连 `plan_digest` 都是空的（那个指纹来自
`/api/story/episode-brief`，只有分集类的页签才有）→ 体检对它们一律 `unknown`，
**任何输入变了都没人说话**。而那四类恰恰是"从零开始创作"真正会用的东西。

**病根**：补 39（提示词）与补 40（润色）把"**模板也是输入**"这条纪律各自钉在了自己的链上，
补 40 的收尾写着"补 38 那张清单终于数完了" —— 可那张清单只数了**数据**那一维
（设定 / 名册 / 正文 / 本集上下文）。**模板是与链正交的另一维**：每条链各有一套模板。
逐维 × 逐链打勾才看得出第一条链（剧本生成，也是整条流水线最贵、最靠前的那条）**漏了模板**。

**修法**（这一维**不需要搬家**，因为模板是服务端库里的一行）：

- `story.templateDigest(tpl)` —— **唯一一份**模板指纹，覆盖 `system + content` 逐字
  （**只哈希 `content` 就是半个指纹** —— 这也正是 B102/B103 尾部那个 `builtin_digest`
  缺口的同一句话在"输入指纹"这一侧的补课）；
- `story.auditScriptTemplate(scripts, {templates})` —— 纯函数，三态 `ok` / `stale` / `unknown`；
- `POST /api/scripts` 记 `template_id` + **服务端算**的 `template_digest`
  （前端传上来的 `template_digest` 一律被覆盖）；`GET /api/scripts` 逐行下发 `template_state`；
  `GET /api/story/audit` 出 `template_issues` + 并进 `issues` + `template_counts` + `template_stamped`；
- 页面上 `resultGenTemplateId` 在**生成成功那一刻**赋值（**保存时不再按当前页签现取** ——
  真实动作顺序是"生成完 → 切走看看 → 回来保存"，现取会记成另一个模板 ＝ 永久假警报）；
  润色产出 / 载入既有 / 清空结果三条路都把它清掉；逐集生成那条路也单独记；
- 出口（`data-regen-tpl`）落在**用这个模板的那个页签**上，分集剧本顺手把**这一集**的素材载回来。

**三个关键决定**：

1. **只判"每个槽位最新的那一条"**（槽位 = 项目 + 页签 + 集号）。不这么收口，用户改一次模板，
   历史上**每一次尝试**都会变成 `stale`，面板刷出一串早被取代的草稿 → **用户学会无视整块标记**
   （补 18 的"部分失败 N 段"就是这么被无视的）。收口之后顺带得到"重生成 → 标记自己消失"，
   不需要任何"已读/忽略"状态。
2. **没记模板的行一条都不报**（本轮之前生成的）：不猜，也不凭空多出一批过期；
   分母 `template_stamped` 如实给出。
3. **改模板的 `name` / `notes` 不许报**（改个名字不该让所有剧本过期）；**只改 `system` 必须报**。

**顺手修的两处**：

- **apitest 的分组求和棘轮从"手抄清单"改成从返回体推导**（`groupWarnSum`）——
  那条棘轮本来就是为了抓"新增一组忘了进总数"，而它自己手抄的组名清单**漏了别的组却照样绿**
  （补 39/40/41 各加一组时都漏了；`drift_counts` 一度也被漏掉，只因那些组恰好是 0）。
- **uitest 那条从"漂移体检"长成"所有分组都要并进 `issues`"的断言改名** ——
  名字不跟着改，它红了会把人指向错的地方。

**教训**：① **"清单数完了"要问清"数的是哪一维"** —— 审计一张清单先把维度列出来
（数据 / 模板 / 名册 / 设定 / 版本），再逐维 × 逐链打勾；只按链走一遍，永远会漏掉
"每条链都有的那一维"；② **"最贵的那条链"值得单独回头看一眼** —— 前两轮修的都是它的下游；
③ **"不需要搬家"也是一种结论**：模板服务端拿得到，所以能**不动那条最贵的链**就修好
（不动 = 不引入新故障）；④ **假警报的形态不止一种**（"改一次模板 → 历史全过期"），
**用户学会无视一整块标记**的代价比漏报一次更大；⑤ **钉短语 ≠ 钉判据**
（注意事项 11 第四次复发）：uitest 第一版钉 `/latest\.get\(key\)/`，而
`const prev = latest.get(key);` 那一行**一直在** —— 把收口逻辑整个换掉，断言**照样绿**；
判别法只有一句：**把它关掉，这条断言会红吗？** 钉的必须是**做决定的那一行**；
⑥ **边界要写"今天为什么不需要"**：`shotsFromText`（分镜生成）的提示词硬编码在代码里、
**没有模板**，所以"模板改了没报"今天不存在；将来它变成模板，就按同一套机制加指纹。

对照 NH–NM 见 `docs/research/09-story-bible-plan.md` §6.41。

## B103 剧本润色：改了来源剧本，润色稿还是旧的、还报到"没过期"（批 8 补 40：清单第 ② 条）

**症状**：故事脚本页点「强化冲突 / 强化开头 3 秒 …」润色一份剧本、点「保存到项目」，
库里多一条记录（**另存一条**）。此后**改掉那条来源剧本的正文**（或改优化模板、或改某个角色的外貌），
这条润色稿**一个字都不变**，而它在 `GET /api/story/staleness` 里是 **`ok`** ——
一个与它的真实来源毫无关系的 `ok`（它带的是 `plan_digest`，那是**本集上下文**的指纹）。
界面上两条都"看起来正常"，没有任何一层会报。

**根因（与 B102 是同一类病的两种形态）**：
① B102 是**原地写回**（改了描述、提示词没变）；这一条是**另存一条**（改了来源、派生稿没变）。
   两者都是"派生字段/派生记录过期"，而剧本/分镜的过期体检答的是"该不该重生成"，
   `shot_prompt_stale` 只看同一行上的派生字段 —— 都照不到"这条记录是从哪儿来的"。
② 病根还是**合成在前端**：`optimize()` 自己拼 `roster.text + tpl.content.replace(/\{\{[^}]+\}\}/g, result)`
   再走 `POST /api/agnes/text`，而该端点把 `messages` **原样转发** —— 服务端看不到发出去的那一份，
   **不可能**给它算指纹（补 36/37/38/39 同一条理由）。

**修法**：`story.polishInput(sourceText, tpl, chars)` 是**唯一一份**合成（指纹覆盖 `system + user`，
`user` 里已含名册 → 三样输入全覆盖），`story.polishInputDigest` 由**写入点与复算点共用**；
`POST /api/scripts/polish` 把**合成 → 调模型 → 回传产物 + 指纹**一次做完（只认 `optimize` 模板；
模型返回空 → 502 且不写）。剧本行新增 `source_script_id` / `source_template_id` / `source_digest`，
**指纹一律服务端算**（前端只说"从哪一条、用哪个模板"，说了不算）；
`story.auditPolishStaleness` 并进体检的 `polish_issues` + `issues`，`GET /api/scripts` 逐行下发
`polish_state`，已保存列表渲染「润色稿」/「来源已变·点此重润」/「未验」，点重润**就地更新**那一条。

**两处刻意的设计决定**：
① **名册按"被润色的那份文本"算**（不按页面上的生成表单字段）：后者服务端**复算不到**，
   而"复算不到的输入进不了指纹"是这一系列的硬边界。这是本轮**唯一**的行为变更。
② **编辑过的草稿不冒充来源**：页面允许改两笔再润色，那不是"从第 7 条派生的"。
   端点比对正文与那一行，逐字相同才认；否则回 `source_script_id: null` → 保存时留空 → 体检说
   `unknown`。**若照记来源，复算出来的指纹永远对不上 = 永久假警报**（对照 NG 实测到这一点）。

**精确性**：`ok` / `stale`（来源变了）/ `stale`（来源被删了，文案不同）/ `unknown`（模板被删、
没记模板、没有指纹）四态分明；不是润色稿的行**一条都不报**；只改 `personality`（不进名册）不许报。

**状态**：已修（批 8 补 40）。对照 NA–NG 见 `docs/research/09-story-bible-plan.md` §6.40。

**顺带记下的遗留（与 B102 尾部同一条）**：`lib/seed.js` 的 `planTemplateSync` 判"这一行还是不是
我们发的那一版"时，digest **只覆盖 `content`、不覆盖 `system`** —— 只改模板 `system` 的官方更新
到不了已有安装。本轮润色链把 `system` 纳入了**输入指纹**（改了 system 会让润色稿过期），
但"模板自身能不能被同步下去"仍是那个缺口，仍未动。

## B102 图片/视频提示词：改了画面描述，英文提示词还是旧的（批 8 补 39：清单第 ① 条）

**症状**：改了分镜的 `scene_description`（或动作、出场人物），那一行的 `image_prompt` / `video_prompt`
**一个字都不变**，出图时模型照着**旧描述**写出来的那份提示词出图。界面上两条都"看起来正常"，
体检也报不出来 —— 剧本/分镜的过期体检看的是"该不该重生成分镜"，与这一行的派生字段无关；
`shot_style_baked` 只看"有没有写死画风词"、`shot_char_unknown` 只看"名字在不在角色库"，
都答不了"这份提示词还是不是按当前内容写的"。

**根因（两件事叠在一起）**：
① 提示词是**派生字段**，由"这一镜自己的内容 + 提示词模板"生成后**原地写回同一行** ——
   这是"输入变了、产物没变"的**第三种形态**（补 36 设定 / 补 37 名册 / 补 38 分镜指纹都表现为
   "该重生成"，这一种表现为"这份派生字段该重做"），前三种的体检**照不到它**。
② 合成在**前端**：`batchPrompts` 把 system 与 user 硬编码在页面里再走 `POST /api/agnes/text`，
   而那个代理端点把 `messages` **原样转发** —— 服务端**看不到**真正发出去的那一份，
   所以**不可能**给它算指纹（与补 36/37/38 同一条理由，只是这次挡在门口的是"合成在前端"）。

**顺带发现的假承诺**：`lib/seed.js` 里一直有 `image_prompt` / `video_prompt` 两个内置模板
（设置页能编辑 system、能改正文变量、还带 `negative_prompt`），而**没有任何一行代码读过它们** ——
真正发出去的是前端那份硬编码。这是补 25 那一类"只被显示、没被读取"，而且是**整个设置分区**级别的。

**修法**：`story.promptInput(shot, kind, tpl, chars)` 是**唯一一份**合成实现，
返回 `{messages, digest, input_chars}`，指纹覆盖 **system + user 逐字**；
`POST /api/storyboards/:id/prompt` 把**合成 → 调模型 → 落库 → 记指纹**一次做完，
前端只逐条推进度 —— **"发出去的"与"进指纹的"由构造保证是同一份**。
模板改成读设置页那两个 key（前端那份 system 删掉），并把补 7 的**三条禁令**搬进模板的 `system`。
模板正文与变量**对齐"服务端真的填得出"的那几个**：删掉 `{{镜头运动}}`（运镜由使用点注入，
写进去就是写两遍）与 `{{情绪目标}}`（**分镜行上没有这个字段**，填不出来会被 `renderPrompt`
换成空串＝静默丢失），换成真的有的台词/旁白/音效。

**精确性**：进提示词的字段改了才报 `stale`；**不进的不许报**（`camera_move`/`negative_prompt`/
`duration_seconds`/`shot_number`/`status` —— 报了就是假警报，比不检查更糟）；模板换了也报 `stale`；
四态分明 `ok`/`stale`/`unknown`/`missing`（**还没生成不报**，那是"还没做"）；模板读不到一律 `unknown`；
**手写提示词也记指纹**（记的是"写它时的输入"，不区分谁写的，否则手写过的行永远报过期）；
**模型返回空不写库并报错**（把失败变成静默清空是最难发现的一类失败）。

**出口**：体检给 `shot_prompt_stale` + `go`，但标记必须在**用户改描述的那一页**上 ——
分镜列表逐行下发 `image_prompt_state`/`video_prompt_state`，提示词列自己标「提示词过期·点此重做」。
**重做入口必须就在标记旁边**：批量补提示词**默认跳过已有提示词**，所以"把过期的那几条重做"
在批量入口里**点不到**（补 15 的同一形状：防护变成陷阱）。

**还剩一条（本轮发现，有意不做）**：补 28 的同步判据 `builtin_digest` **只覆盖 `content`，不覆盖 `system`** ——
所以"只改 system、不改 content"的那次改进**到不了老用户手里**（本轮因为 content 一起改了才升上来）。
它动的是**唯一一处会覆盖用户数据的逻辑**，风险与收益不对等，单开一轮做：
改判据为 `content + system` 的合成指纹，并让 `ours` 判据同时接受"行上的旧指纹在历史表里"
（否则老库会被判成"用户改过"而永不更新），历史表要同时登记**旧方案的 content-only 指纹**。

**教训**：① **"派生字段"要单独想一遍** —— 与"该重生成"混在一起想，会得出"已经有体检了"的错误结论；
② **调研要顺手问"这东西真的被读过吗"** —— 两个模板摆了好几轮，一次"谁读了这个 key"的检索
比读十遍代码便宜；③ **搬家的理由这次是"指纹需要看到它"**；④ **出口要放在用户动作发生的那一页**；
⑤ **"模板也是输入"**；⑥ 注释里引用旧变量名会让"整段匹配"的断言打在注释上（本轮真踩到）——
断言要落在**真正发出去的那一行**。

---

## B101 AGENTS.md 撞上工作区指令体积预算（长期问题，本轮结构性解决）

**症状**：`AGENTS.md` 是每轮都要追加条目的工作指南（本轮之前已经写到 65106 B），而工作区指令有
**65536 B 的硬预算** —— 超了会被**静默截断**（截断点在文件中部，被切掉的那半节再也读不到）。
于是最近几轮出现了同一个循环：**要加一条新条目，就得先"挤"掉旧条目的若干字**。
本轮一开始也是这么干的（压 补 33/34/35、补 27、补 36、补 31 的措辞，挤出约 1.5 KB），
但挤完发现：新增的 补 38 条目（约 1.7 KB）还是塞不下，而且**下一轮会再来一次**。

**根因**：不是"某一条写得太长"，而是**主指令里长期放着一节"只在极少数时刻才用得上"的操作手册** ——
「知识图谱（`.understand-anything/`）」那一节里的**重建代价与做法**（约 2 KB）只在**真要重建图谱**时才有用，
其余每一轮都在为它付体积。**把"偶尔用的长手册"和"每轮都读的规则"放在同一个有预算的容器里，就一定会互相挤。**

**修法（搬家优于删除，也优于压缩）**：把「重建的代价与做法」整节**原样**抽到
`docs/research/10-knowledge-graph-rebuild.md`（连同"何时需要读"的前言），主指令那一节只留**一段指针**
（点名四件最重要的事 + "动手前先读那一份"）。结果：AGENTS.md 65106 → **64402 B**，
既没有丢任何内容，也不用再压缩别人的措辞，而且**下一轮加条目时不用再挤**。

**教训**：① **有体积预算的文档要按"读取频率"分层** —— 每轮都读的（规则、约定、注意事项）留在主指令，
偶尔才读的（操作手册、实测过程）放 `docs/` 并从主指令**指过去**；② **遇到"又要挤掉一点"时先问
"这是不是结构问题"** —— 连着几轮都在做同一件体力活，通常说明有东西放错了容器，而不是措辞不够精炼；
③ 搬家时**原样搬**（不做二次改写），这样"内容没丢"是可核对的（本轮抽出的 2046 B 与原文逐字相同）。

**附带的一条纪律**：主指令里的**四套测试计数**（selftest / apitest / uitest / browser-test）每轮都要同步 ——
它是"这一轮的测试真的加上了"的最便宜的证据，忘了改就等于文档在说一个过时的数。

---

## B100 分镜的输入指纹漏了名册（批 8 补 38：清单上的第二处，同一类病的下一层）

**症状**：往角色库里**新增一个角色**、或改了某个角色的**外貌**之后，已经生成的分镜**一行都不会变**，
「过期体检」照样显示 `ok`。可分镜提示词真正发出去的是 `角色名册 + 剧本正文`（补 6 加的），
名册变了 → 模型下次写出来的「出场人物」也会变（新角色会被用本名写出来，进而**绑得上档案、拿得到外貌注入**）
—— 也就是说：**输入已经变了，指纹没变**。

**根因**：分镜的过期判定只比 `digestText(剧本正文)`（补 12 的 `script_digest`），而名册不在里面。
这与 B98（设定漏了剧本指纹）、B99（名册漏了剧本指纹）**完全同型**，只是发生在"剧本 → 分镜"这一层。

**危害是实的、而且没有任何一层会报**：老分镜里模型写的是"少女"这类代称（它当时不知道名册），
于是 `characters` 绑不上任何档案 → **没有外貌注入**；而体检的 `shot_char_unknown` 带**泛称停用词表**，
"少女"这类词被有意排除（否则会刷满假警报）—— 于是这一路**静默到底**。

**修法**：新增唯一实现 `story.storyboardInputDigest(script, chars)`，**两个调用点共用**：
① 入库那一刻（`lib/routes.js` 的 `sourceDigest`）；② 体检复算（`lib/story.js` 的 `auditStaleness`）。
名册**按这份剧本所属项目**取角色。**前端一行都不用改** —— 前端 `loadRoster(text)` 的输入文本就是这份剧本正文，
服务端用同一个正文调同一个 `characterRoster`，两边必然逐字相同（"同一件事只有一份实现"的直接红利，
连"让前端把名册回报上来"这种新协议都不需要）。

**精确性**：没有角色时**不 push 空串**，指纹与从前逐字节相同 —— 补 38 之前生成的分镜照样 `ok`；
只改 `personality`/`role`（不进名册）**不许**报过期（配前提断言证明 PUT 真写进去了）；
删光角色后指纹**逐字节回到最初那一版**。

**反事实（会红的那种）**：判定侧漏挂 `characters` 时，复算退回"只哈希正文"，与入库那份不是同一个东西
→ **每一集永久报过期**（假警报），**不是**静默 `ok`。第一版断言写反了（以为会报 ok），跑出来是 `stale` ——
按**真实**失败模式改了断言，没按预想去改代码。

**顺带补上一个从没被钉过的前提**：`shotsFromText(text, ep, sourceScriptId)` 少传第三个参数**不报任何错**，
但整批分镜的指纹会变成空串 → 过期体检对它们**永远只能说 `unknown`** → "剧本改了、分镜还是旧的"再也报不出来。
四套测试此前**没有一条**钉过它。现在真机断言：逐集生成的分镜，`source_script_id` 必须等于那一集的剧本 id、
`script_digest` 必须是 8 位十六进制、且该集在过期体检里是 `ok`。

**清单上还剩两条（记下来别丢）**：① 图片/视频提示词是从分镜行自己的字段生成的、**原地写回同一行** ——
"画面描述改了、英文提示词还是旧的"没有任何一层会报（"输入变了产物没变"的第三种形态：不是"该重生成"，
而是"这份派生字段过期了"）；② 剧本润色另存一条剧本，它的 `plan_digest` 该记哪一份要单独想清楚
（润色**没有改变输入**，所以润色后的新剧本同样该是 `ok`，这条要钉）。

**教训**：① **"清单"要真的列出来** —— 列出来的那一刻，"两处已覆盖、一处漏了"是一目了然的，
靠"再猜一处"只能一个个撞；② **清单要先回答"服务端能不能拿到这份输入"**：拿不到的输入**不可能**进指纹，
要么搬到服务端，要么承认这条链没有过期判定；③ **失败模式想反了就改断言、不是改代码**；
