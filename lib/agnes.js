...
const story = require('./story');   // 负面提示词措辞与图片链同源（story.negativePhrase）
...
function v25Body(model, params) {
  const body = { model };
  body.prompt = story.negativePhrase(params.prompt, params.negative_prompt);
 ...
 */
/**
 * R27：把上游的**真实扣费**提取出来落库。
 *
 * 为什么不能只在界面上按条数估：预估是"我们以为要花多少"，扣费是"实际花了多少"，
 * 两者不一致时用户只会信任后者，而现在的库表里连后者都没有。
 * 但 Agnes 是否返回费用字段**未确认**（研读报告的判断），所以这里做的是"有就存、没有就 null"：
 * **绝不用 0 冒充"免费"**——竞品正是因为 `estimatedPrice=0` 的语义模糊（免费还是未知？）
 * 不得不写专门的降级文案，我们不去继承这个歧义。`null` = 未知，数字 = 上游说的数。
 *
 * 只扫顶层与 `usage`/`data` 这两层（不递归整棵树）：深层随便一个叫 amount 的数字
 * 都可能是别的含义，宁可漏也不要把无关数字当钱存进去。
 */
// 变更须知：`null` = 上游没给（未知），`0` = 上游明确说免费，两者不可互相顶替；
// 界面对 null 不显示任何数字。把缺失值兜成 0 会让用户以为免费，这是竞品踩过的坑。
const COST_CREDIT_KEYS = ['credits', 'credit', 'points', 'point', 'coins', 'coin'];
const COST_MONEY_KEYS = ['cost', 'price', 'fee', 'amount', 'total_cost', 'cost_amount'];
function extractCost(result) {
  const out = { cost_credits: null, cost_amount: null, cost_unit: null };
  if (!result || typeof result !== 'object') return out;
  const scopes = [result, result.usage, result.data].filter((x) => x && typeof x === 'object');
  for (const sc of scopes) {
    for (const k of COST_CREDIT_KEYS) {
      const v = Number(sc[k]);
      if (sc[k] != null && Number.isFinite(v)) { out.cost_credits = v; out.cost_unit = 'credits'; return out; }
    }
  }
  for (const sc of scopes) {
    for (const k of COST_MONEY_KEYS) {
      const v = Number(sc[k]);
      if (sc[k] != null && Number.isFinite(v)) { out.cost_amount = v; out.cost_unit = 'currency'; return out; }
    }
  }
  return out;
}
...
function buildSafeStatusUpdate(result) {
  const updates = { raw_status_response: result, ...extractCost(result) };
...
module.exports = {
  ...
  extractVideoUrl, buildSafeStatusUpdate, extractCost,
  testConnection,
};
