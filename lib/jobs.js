...
/**
 * 追加一个"收尾阶段"占位项（例：原著解析在全部文本块抽完之后还有一次「全局归并」调用）。
 * 变更须知：队列跑完但任务还没完的场景，必须把收尾阶段也挂进 items —— 否则界面显示
 * 12/12 已完成、任务却仍在跑，看起来像卡死（用户会去点取消，白烧一次配额）。
 * 由调用方负责改它的 state / ok / error，并同步 job.done（这里不动计数，避免重复计）。
 */
function appendItem(job, label, key = null) {
  const rec = {
...
module.exports = { create, get, list, run, cancel, prune, appendItem };
