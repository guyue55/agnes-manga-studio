...
async function pollOnce(assetId) {
  ...
  if (justCompleted && merged.video_url && merged.storyboard_id) {
    ...
    if (sb && sb.linked_video_id !== merged.id) store.update('storyboards', merged.storyboard_id, { linked_video_id: merged.id });
  }
  // 3.3：SSE 附带轮询节奏视图字段（内存计数，不落库、重启归零），任务页据此显示"第 n 次查询"
  events.emit('video', { ...merged, poll_attempts: (counts.get(assetId) || 0) + 1, poll_interval_s: Math.round(intervalMs() / 1000) });
  return merged;
}
