...
module.exports = function createRoutes(ctx) {
   ...
   */
  on('POST', '/api/story/look-fill', async (req, res, params, body) => {
    const source = store.get('story_sources', str(body.source_id));
    if (!source) throw httpError(404, '原著不存在');
    const cards = store.list('story_cards', { filter: (r) => r.source_id === source.id });
    const { chars, targets } = story.lookFillTargets(cards);
    if (!chars.length) throw httpError(400, '这份原著还没有人物卡：先解析出人物卡，才有长相可以补');
    if (!targets.length) {
      return { ok: true, targets: 0, chars: chars.length, assigned: 0, calls: 0,
        note: '所有人物卡都已经有外貌或服装了' };
    }
    // 出处来自解析时那套参数重新切块（补 18 的 chunk_opts 纪律：切不回原样就不动手）
    const { split, aligned } = reSplitSource(source);
    const excerpts = story.lookFillExcerpts(targets, split.chunks);
    const withText = excerpts.filter((e) => e.has_text);
    // 干跑：先让界面把"要补几张、调几次"如实告诉用户，确认了再花钱
    if (bool(body.dry_run)) {
      return { ok: true, dry_run: true, targets: targets.length, chars: chars.length,
        with_source: withText.length, no_source: targets.length - withText.length, calls: 1, aligned };
    }
    if (!withText.length) {
      return { ok: true, assigned: 0, targets: targets.length, chars: chars.length, calls: 0,
        no_source: targets.length, not_found: [], ungrounded: [], missing: [], invalid: [],
        note: '这些人物卡都没有可定位的原文出处（人工新建或来自全局归并没有段号）——回原文找无从谈起，卡片未被改动' };
    }
    const tpl = storyTemplate('char_look');
    const r = await storyAsk(tpl, { 人物与原文片段: story.lookFillLines(withText) },
      { model: body.model, project_id: source.project_id, note: '原著解析 · 补人物长相' });
    if (!r.ok) throw new Error(r.error);
    const byId = {};
    withText.forEach((e) => { byId[e.id] = e.text; });
    const plan = story.applyLookAssignments(withText, r.parsed, byId);
    ...
    return {
      ok: true, assigned: plan.patch.length, targets: targets.length, chars: chars.length, calls: 1,
      with_source: withText.length, no_source: noSource, aligned,
      ...
      note: plan.patch.length ? '' : '模型没有给出可用的长相（原文里没写、或引文对不上原文）——卡片未被改动',
    };
  });
