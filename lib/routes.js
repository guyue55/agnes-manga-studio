...
module.exports = function createRoutes(ctx) {
  ...
  on('GET', '/api/scripts', (req, res, params, body, query) => {
    ...
    const rows = store.list('scripts', {
      filter: (r) => (projectId ? r.project_id === projectId : true),
      sort: (a, b) => String(b.created_at).localeCompare(String(a.created_at)),
    });
    // 批 8 补 40：润色稿的过期**逐行下发**（同补 39 的分镜提示词）。判定要看来源剧本，
    // 而来源可能在别的项目里 —— 所以判定用全量，输出仍只给当前项目这些行。
    // 没有 `source_script_id` 的行拿不到状态（不是润色稿），界面据此不显示任何标记。
    const allRows = store.list('scripts', { limit: 100000 });
    const allTpls = store.list('prompt_templates', { limit: 100000 });
    const audit = story.auditPolishStaleness(allRows, {
      characters: projectId ? projectCharacters(projectId) : store.list('characters', { limit: 100000 }),
      templates: allTpls,
    });
    // 批 8 补 41：模板那一维逐行下发。判定要按**槽位**（项目+页签+集号）收成最新一条，
    // 所以必须拿全量行算，输出仍只给当前项目这些行
    const tplAudit = story.auditScriptTemplate(allRows, { templates: allTpls });
    return rows.map((r) => Object.assign({}, r, {
      polish_state: audit.states[r.id] || '',
      template_state: tplAudit.states[r.id] || '',
    }));
  });

  on('POST', '/api/scripts', (req, res, params, body) => {
    if (!str(body.content).trim()) throw httpError(400, '剧本内容不能为空');
    // 批 8 补 40：润色是"另存一条"，所以要记下**它是从哪一份、用哪个模板**派生的。
    // 来源必须是**已存在的剧本** —— 页面上的草稿可以随便编辑，但"我是从第 7 条派生的"
    // 这句话要么指向一条真记录，要么就别说（编辑过的草稿不冒充某一行的内容，
    // 否则判定时按那一行复算出来的指纹永远对不上 = 永久假警报）。
    const srcId = str(body.source_script_id).trim();
    if (srcId && !store.get('scripts', srcId)) {
      throw httpError(400, '来源剧本不存在（润色稿必须指向一条真的剧本记录）');
    }
    const row = {
      ...
      source_script_id: srcId,
      source_template_id: str(body.source_template_id).trim(),
      source_digest: '',
      // 批 8 补 41：**生成模板也是输入**。前端只报"我用的是哪个模板"（id），
      // 指纹由服务端按**它自己库里那份模板**算 —— 模板是可编辑的，判定时用同一个函数复算才同源。
      template_id: str(body.template_id).trim(),
      template_digest: '',
    };
    const genTpl = row.template_id ? store.get('prompt_templates', row.template_id) : null;
    row.template_digest = genTpl ? story.templateDigest(genTpl) : '';
    stampPolishDigest(row, row); // 指纹服务端算（绝不用前端传的）
    return store.insert('scripts', row);
  });

  /**
   * 润色一条剧本（批 8 补 40）：**合成 → 调模型**都在服务端做完。
   *
   * 为什么必须搬过来：润色的输入是"来源文本 + optimize 模板 + 角色名册"，而
   * `POST /api/agnes/text` 把 `messages` **原样转发** —— 合成留在页面上，服务端就**看不到**
   * 真正发出去的那一份，也就**不可能**给它算指纹（补 36/37/38/39 同一条理由）。
   *
   * 回传 `source_matches_row`：来源文本是否**逐字等于** `source_script_id` 那一行。
   * 用户可以在结果区改两笔再润色，那不是"从第 7 条派生的" —— 服务端如实说清，
   * 页面据此决定要不要记来源（不记就是 `unknown`，而不是按第 7 条复算出**永远对不上**的指纹）。
   */
  on('POST', '/api/scripts/polish', async (req, res, params, body) => {
    const srcText = str(body.source_text);
    if (!srcText.trim()) throw httpError(400, '没有可润色的内容（结果区是空的）');
    const tpl = tryPolishTemplate(body.template_id);
    if (!tpl) throw httpError(404, '优化模板不存在（或它不是优化类模板）');
    const projectId = str(body.project_id).trim();
    const input = story.polishInput(srcText, tpl, projectCharacters(projectId));
    const wantId = str(body.source_script_id).trim();
    const srcRow = wantId ? store.get('scripts', wantId) : null;
    const matches = !!(srcRow && str(srcRow.content) === srcText);
    const r = await fetchInternal('POST', '/api/agnes/text', {
      messages: input.messages,
      project_id: projectId || null,
      note: str(tpl.name),
    });
    if (!r || r.ok === false) throw httpError(502, (r && r.error) || '模型调用失败');
    const content = str(r.content).trim();
    // 空输出**不写**：把"这次没成功"变成一次静默清空是最难发现的一类失败（补 39 同一条）
    if (!content) throw httpError(502, '模型返回了空内容（结果区保持原样，可重跑）');
    return {
      ok: true, content, digest: input.digest,
      template_id: tpl.id, template_name: str(tpl.name),
      source_script_id: matches ? srcRow.id : null, source_matches_row: matches,
      source_chars: srcText.length,
      roster_count: input.roster_count, roster_chars: input.roster_text.length,
      input_chars: input.input_chars, model_said_chars: str(r.content).length,
    };
  });

  on('PUT', '/api/scripts/:id', (req, res, params, body) => {
    const patch = {};
    for (const k of ['title', 'content', 'script_type', 'project_id', 'plan_digest', 'source_script_id', 'source_template_id']) if (k in body) patch[k] = str(body[k]);
    if ('episode_number' in body) patch.episode_number = Math.max(0, num(body.episode_number, 0));
    const before = store.get('scripts', params.id);
    if (!before) throw httpError(404, '剧本不存在');
    const merged = Object.assign({}, before, patch);
    if (str(merged.source_script_id).trim() && !store.get('scripts', str(merged.source_script_id).trim())) {
      throw httpError(400, '来源剧本不存在（不能把润色稿指向一份不存在的剧本）');
    }
    // 正文/来源/模板任一变都要重算：改了正文就不再是"从那份来源派生"的了
    stampPolishDigest(merged, patch);
  ...
  on('GET', '/api/storyboards', (req, res, params, body, query) => {
    ...
    const rows = store.list('storyboards', {
      filter: (r) => (projectId ? r.project_id === projectId : true)
    ...
    // 批 8 补 39：逐行带上"提示词是否过期"。提示词就在这一页生成、也就在这一页上改画面描述 ——
    // 把结论只放在别处的体检页，等于要求用户自己记住"我改过描述"（补 25 的教训：只被显示的标记等于没有）。
    const promptAudit = story.auditPromptStaleness(rows, {
      characters: projectId ? projectCharacters(projectId) : store.list('characters', { limit: 100000 }),
      templates: { image: tryPromptTemplate('image'), video: tryPromptTemplate('video') },
    });
    return rows.map((r) => Object.assign({}, r, promptAudit.states[r.id] || {}));
  });
  ...
  function storyboardRow(r, idx) {
    ...
    camera_move: cameraMovePhrase(r.camera_move) ? str(r.camera_move).trim() : '',
      ...
      // 批 8 补 39：提示词的**输入指纹**（"写它的时候输入长什么样"）。字段形状必须在这里出现，
      // 值由 stampPromptDigests 填 —— 它与提示词同一次写入落下，所以不可能只有一个而没有另一个。
      image_prompt_digest: str(r.image_prompt_digest),
      video_prompt_digest: str(r.video_prompt_digest),
    };
  }

  /**
   * 某份剧本的**分镜输入指纹**；剧本不存在时为空串（"不知道来源"要能与"内容一致"区分开）。
   *
   * 批 8 补 38：这里从前只哈希正文，而分镜提示词真正发出去的是 `角色名册 + 正文` ——
   * 于是往角色库加个人/改长相，模型下次会看到不同的名册，指纹却一个字都不变（补 36/37 同类病）。
   * 名册必须按**这份剧本所属项目**的角色算，与前端 `loadRoster(正文)` 那一份同源
   * （两边都用这份正文当输入文本，所以结果逐字相同）。
   */
  function sourceDigest(scriptId) {
    ...
    if (!sc) return '';
    return story.storyboardInputDigest(sc, store.list('characters', {
      filter: (c) => c.project_id === sc.project_id, limit: 100000,
    }));
  }
  ...
  on('POST', '/api/storyboards', (req, res, params, body) => {
    if (Array.isArray(body.rows)) {
      ...
      // 批 8 补 39：新建时若已经带着提示词（粘贴/导入），同样要记下它的输入指纹 ——
      // 否则这些行的体检永远只能说 `unknown`，而它们恰恰是最容易与描述对不上的一批
      const mapped = body.rows.map(storyboardRow);
      for (const row of mapped) stampPromptDigests(row, row);
      const rows = store.insertMany('storyboards', mapped);
      return { inserted: rows.length, rows };
    }
    const row = storyboardRow(body, 0);
    stampPromptDigests(row, row);
    return store.insert('storyboards', row);
  });
  ...
  on('PUT', '/api/storyboards/:id', (req, res, params, body) => {
    ...
    // 批 8 补 39：只要这一行写了提示词，就顺手记下"写它的时候输入长什么样" ——
    // 手写的也算（记的是**输入**，不是"谁写的"），这样体检从这一刻起重新有效，
    // 而不是对着手写内容永远报过期（假警报比不检查更糟）。模板读不到就不记（宁可 unknown，也不猜）。
    stampPromptDigests(Object.assign({}, store.get('storyboards', params.id) || {}, patch), patch);
  ...
  /**
   * 提示词模板（批 8 补 39）：**不抛异常**的取法。
   *
   * 生成那条路用 `storyTemplate`（缺模板要明确报错让用户去设置页补）；
   * 但"写提示词时顺手记指纹"是**副作用**，不能因为模板被删掉就让一次普通的保存失败 ——
   * 读不到就返回 null，调用方据此把指纹留空（体检会说 `unknown`，而不是拦下这次保存）。
   */
  function tryPromptTemplate(kind) {
    const key = story.promptField(kind) === 'video_prompt' ? 'video_prompt' : 'image_prompt';
    return store.list('prompt_templates', { filter: (r) => r.key === key })[0] || null;
  }

  /** 该项目某份分镜的角色（名册/提示词都按**分镜所属项目**取，跨项目不互相干扰） */
  function projectCharacters(projectId) {
    return store.list('characters', { filter: (c) => c.project_id === projectId, limit: 100000 });
  }

  /**
   * 把"这一行提示词的输入指纹"写进 patch（批 8 补 39）。
   *
   * `row` 是**打上 patch 之后**的行（复算必须按写完的状态），`patch` 用来判断这次到底写了哪个字段。
   * 只在真的写了 `image_prompt`/`video_prompt` 时才记 —— 改画面描述**不**顺手改指纹：
   * 那正是"过期"要表达的事（描述变了、提示词没变）。
   */
  function stampPromptDigests(row, patch) {
    for (const kind of story.PROMPT_KINDS) {
      const field = story.promptField(kind);
      if (!(field in patch)) continue;
      if (!str(row[field]).trim()) { patch[story.promptDigestField(kind)] = ''; continue; }
      const tpl = tryPromptTemplate(kind);
      if (!tpl) continue;
      patch[story.promptDigestField(kind)] = story.promptInput(row, kind, tpl, projectCharacters(row.project_id)).digest;
    }
  }

  /**
   * 按 id 找一个 **optimize** 模板（批 8 补 40）。
   *
   * 只认 `template_type === 'optimize'`：润色链只该用优化模板，拿"分镜生成"的模板去润色
   * 会把整份剧本当成分集骨架塞进去，产物看着像模像样、实际是错的 —— 这类"用错模板"
   * 在界面上看不出来，所以在入口就挡掉。
   */
  function tryPolishTemplate(id) {
    const key = str(id).trim();
    if (!key) return null;
    const t = store.get('prompt_templates', key);
    return t && t.template_type === 'optimize' ? t : null;
  }

  /**
   * 把"这份润色稿是从哪儿派生的"的指纹写进 patch（批 8 补 40）。
   *
   * **指纹一律服务端算，绝不收前端传的**（同补 38/39）：前端只说"我是从哪一份、用哪个模板润色的"，
   * 指纹由服务端按**它自己库里那份来源正文 + 模板 + 名册**算 —— 这样判定时用同一个函数复算，
   * 两者必然同源。前端算一遍再传上来的话，来源一改、两边的口径就会分叉（补 37 的病根）。
   */
  function stampPolishDigest(row, patch) {
    const srcId = str((patch && 'source_script_id' in patch ? patch.source_script_id : row && row.source_script_id)).trim();
    if (!srcId) return; // 不是润色稿（生成/手写）：本条不记，补 12 的过期体检管那种
    const src = store.get('scripts', srcId);
    const tplId = str((patch && 'source_template_id' in patch ? patch.source_template_id : row && row.source_template_id)).trim();
    const tpl = tryPolishTemplate(tplId);
    // 读不到来源或模板就留空 → 体检说 `unknown`，而不是拦下一次保存（同 tryPromptTemplate 的理由）
    patch.source_digest = (src && tpl) ? story.polishInputDigest(src.content, tpl, projectCharacters(src.project_id)) : '';
  }

  /**
   * 生成一条提示词（批 8 补 39）：**合成 → 调模型 → 落库 → 记指纹**全在服务端一次做完。
   *
   * 为什么不在前端拼：`POST /api/agnes/text` 把 `messages` **原样转发**，前端拼的话服务端
   * 看不到真正发出去的那一份，也就不可能给它算指纹（补 36/37/38 同一条理由）。
   * 合成搬过来之后，"发出去的"与"进指纹的"**由构造保证是同一份**。
   * 前端仍然逐条调用（进度条与取消照旧），服务端负责每一次调用的输入与落库。
   */
  on('POST', '/api/storyboards/:id/prompt', async (req, res, params, body) => {
    const kind = str(body.kind) === 'video' ? 'video' : 'image';
    const sb = store.get('storyboards', params.id);
    if (!sb) throw httpError(404, '分镜不存在');
    const tpl = storyTemplate(kind === 'video' ? 'video_prompt' : 'image_prompt');
    const input = story.promptInput(sb, kind, tpl, projectCharacters(sb.project_id));
    const r = await fetchInternal('POST', '/api/agnes/text', {
      messages: input.messages,
      project_id: sb.project_id,
      storyboard_id: sb.id,
      note: kind === 'image' ? '分镜图片提示词' : '分镜视频提示词',
    });
    if (!r || r.ok === false) throw httpError(502, (r && r.error) || '模型调用失败');
    // 模型偶尔会把整条提示词包在引号里（前端此前也在做同一件事）——去掉首尾引号再落库
    const text = str(r.content).trim().replace(/^["']|["']$/g, '');
    // 空输出**不写**：把"这次没成功"变成一次静默清空是最难发现的一类失败
    if (!text) throw httpError(502, '模型返回了空提示词（保持原样，可重跑）');
    const patch = { [story.promptField(kind)]: text };
    stampPromptDigests(Object.assign({}, sb, patch), patch);
    const row = store.update('storyboards', sb.id, patch);
    return {
      ok: true, kind, storyboard_id: sb.id, shot_number: sb.shot_number,
      prompt: text, digest: patch[story.promptDigestField(kind)] || '',
      input_chars: input.input_chars, model_said_chars: str(r.content).length, row,
    };
  });
   ...
   */
  // 角色字段的**唯一漏斗**（批 8 补 31）。
  // 起因：角色一共有 **7 个写入点**，各自只 clip 了一部分字段、数字还不一样 ——
  // `notes` 出现过 400 与 500 两种，`alias` 在"人物卡导入"与"体检同步"两条路上**完全没截**。
  // 只要有一个入口不过尺子，前面所有的截断都只是"看起来有"（补 30 的教训：先数清楚有几条写入路径）。
  // 现在写角色只有 insertCharacter / updateCharacter 两个出口，尺子在里面，绕不过去。
  // 字段名清单也来自 story.js（`CHARACTER_EDITABLE_FIELDS`），不再手抄。
  function clipCharacterPatch(patch, cut) {
    const src = patch || {};
    const out = {};
    for (const k of story.CHARACTER_EDITABLE_FIELDS) {
      if (!(k in src)) continue;
      const raw = str(src[k]).trim();
      const v = story.clipCharacterField(src[k], k);
      if (cut && v.length < raw.length) cut.push(k);
      out[k] = v;
    }
    // 非文本字段（is_locked / reference_image_ids / story_card_id / project_id）原样带过
    for (const k of Object.keys(src)) {
      if (story.CHARACTER_EDITABLE_FIELDS.includes(k)) continue;
      out[k] = src[k];
    }
    return out;
  }
  function insertCharacter(obj, cut) {
    return store.insert('characters', clipCharacterPatch(obj, cut));
  }
  function updateCharacter(id, patch, cut) {
    return store.update('characters', id, clipCharacterPatch(patch, cut));
  }

  // `cut`（可选）：把被截断的字段名收集起来交给调用方**如实上报**（批 8 补 31）。
  // 这里以前只做 `str().trim()`，**任何路径都没有上限** —— 而角色的 appearance/outfit 会被
  // `characterPhrase` **原文**注入每一次出图提示词（实测存 20000 字就注入 20000 字），
  // 一个粘错的长文本足以把整条出图链撑爆，且两条路各自看都"正常"。
  function characterFields(b, cut) {
    const picked = {};
    for (const k of story.CHARACTER_EDITABLE_FIELDS) if (k in b) picked[k] = b[k];
    if ('is_locked' in b) picked.is_locked = bool(b.is_locked);
    if ('reference_image_ids' in b) picked.reference_image_ids = idList(b.reference_image_ids, 12);
    return clipCharacterPatch(picked, cut);
   ...
   * 为什么值得做：幕次是**分集的依据** —— 一拍都没标时 `planEpisodes` 会退化成"按拍数平均切"
   * （切点可能落在一幕中间），而"给 30 个拍点标幕次"是纯分类，正是该由模型干、不该由人一格一格填的活。
   * 三条纪律：① 只补"待定"的，**人定过的幕次一个都不动**；② 只认四幕词表，脏值丢弃并上报，绝不落库；
   * ③ 模型可能把幕次写倒退 —— 不替它"修顺"（那是我们编数据），只把倒退的位置报出来。
   * 只调**一次**模型（全部拍点一次给完），所以计费闸门就是"要补几拍"这一个数。
   */
  on('POST', '/api/story/stage-fill', async (req, res, params, body) => {
    const source = store.get('story_sources', str(body.source_id));
    if (!source) throw httpError(404, '原著不存在');
    const cards = store.list('story_cards', { filter: (r) => r.source_id === source.id });
    const { beats, targets } = story.stageFillTargets(cards);
    const before = story.planEpisodes(cards);
    if (!beats.length) throw httpError(400, '这份原著还没有剧情卡：先解析出剧情卡，才有拍点可以分幕');
    if (!targets.length) {
      return { ok: true, targets: 0, beats: beats.length, assigned: 0, calls: 0,
        basis_before: before.basis, basis_after: before.basis, note: '所有剧情拍点都已经有幕次了' };
    }
    // 干跑：先让界面把"要补几拍、调几次模型"如实告诉用户，确认了再花钱
    if (bool(body.dry_run)) {
      return { ok: true, dry_run: true, targets: targets.length, beats: beats.length, calls: 1, basis_before: before.basis };
    }
    const tpl = storyTemplate('plot_stage');
    const r = await storyAsk(tpl, { 拍点清单: story.stageFillLines(beats) },
      { model: body.model, project_id: source.project_id, note: '原著解析 · 补分幕次' });
    if (!r.ok) throw new Error(r.error);
    const plan = story.applyStageAssignments(cards, r.parsed);
    if (!plan.patch.length) {
      // 一条都没接住要**说清原因**（模型返回的 stage 不合词表 / 没给全），不能只报"补了 0 个"
      return { ok: true, assigned: 0, targets: plan.targets, beats: plan.beats, calls: 1,
        invalid: plan.invalid, missing: plan.missing, no_id: plan.no_id,
        basis_before: before.basis, basis_after: before.basis,
        note: '模型没有给出可用的幕次（返回的 stage 不在 起/承/转/合 之内，或没给全）——卡片未被改动' };
    }
    for (const p of plan.patch) store.update('story_cards', p.id, { stage: p.stage });
    const after = story.planEpisodes(store.list('story_cards', { filter: (r2) => r2.source_id === source.id }));
    return {
      ok: true, assigned: plan.patch.length, targets: plan.targets, beats: plan.beats, calls: 1,
      invalid: plan.invalid, missing: plan.missing, no_id: plan.no_id, back_steps: plan.back_steps,
      assigned_names: plan.patch.map((x) => `${x.name}→${x.stage}`),
      // 这一对是这次调用**到底有没有用**的证据：补完之后分集依据应当从 count 变成 stage
      basis_before: before.basis, basis_after: after.basis,
    };
  });
   ...
   *
   * 体检一直把"人物卡没有外貌与服装"报成 `char_no_look` / `fixable: false`（"需要人写"）——
   * 但**原著通常写了**这个人的长相，只是抽取那一步漏了。所以"回原文再找一遍"是**抽取**，不是创作；
   * 真正该防的是"模型凭名字编一个"，于是这一趟的**核心机制是引文可验证**：
   * 模型必须交出原文里的原话，我们把它与那张卡自己的片段逐字比对，对不上的一律丢弃（`ungrounded`）。
   * 原文确实没写时模型回 `found:false` —— 那是诚实的结论，计进 `not_found` 而不是失败，更不拿编的顶上。
   */
  // 回原文补字段（批 8 补 33 补长相，补 34 泛化到地点/道具的可注入字段）。
  // **一个端点、一份实现、按 `target` 取规格**：补长相与补场景字段的机制完全相同
  // （回原文找 → 交原文原话 → 逐字核对 → 对不上就丢），写成两个端点就是两份会分叉的真相。
  on('POST', '/api/story/field-fill', async (req, res, params, body) => {
    const spec = story.fillSpec(body.target);
    if (!spec) throw httpError(400, `不认识的补字段目标「${str(body.target)}」（可用：${Object.keys(story.FILL_SPECS).join('、')}）`);
    const source = store.get('story_sources', str(body.source_id));
    if (!source) throw httpError(404, '原著不存在');
    const cards = store.list('story_cards', { filter: (r) => r.source_id === source.id });
    const { pool, targets } = story.fillTargets(spec, cards);
    const empty = { ok: true, target: spec.key, assigned: 0, pool: pool.length, targets: targets.length,
      with_source: 0, no_source: 0, calls: 0, aligned: true, not_found: [], ungrounded: [],
      empty: [], missing: [], invalid: [], no_id: 0, assigned_items: [], assigned_names: [] };
    if (!pool.length) {
      throw httpError(400, `这份原著还没有${spec.label}相关的卡片：先解析出卡片，才有字段可以补`);
    }
    if (!targets.length) return { ...empty, note: spec.emptyNote };
    // 出处来自解析时那套参数重新切块（补 18 的 chunk_opts 纪律：切不回原样就不动手）
    const { split, aligned } = reSplitSource(source);
    const excerpts = story.fillExcerpts(spec, targets, split.chunks);
    const withText = excerpts.filter((e) => e.has_text);
    // 干跑：先让界面把"要补几张、调几次"如实告诉用户，确认了再花钱
    if (bool(body.dry_run)) {
      return { ...empty, dry_run: true, targets: targets.length, pool: pool.length,
        with_source: withText.length, no_source: targets.length - withText.length, calls: 1, aligned };
    }
    if (!withText.length) {
      return { ...empty, targets: targets.length, pool: pool.length, no_source: targets.length, aligned,
        note: `这些${spec.label}卡片都没有可定位的原文出处（人工新建或来自全局归并没有段号）——回原文找无从谈起，卡片未被改动` };
    }
    const tpl = storyTemplate(spec.template);
    // 变量名对不上是**静默**失败：`renderPrompt` 会把"没给的 {{…}}"替换成**空串**，
    // 于是卡片清单整个发不出去、模型只能凭名字编，而链路上没有任何报错
    // （补 34 真踩到：统一端点时把变量名写成了通用的那个，而 char_look 的模板里还写着 {{人物与原文片段}}，
    // 顺利路径直接全空 —— 三条断言同时红，但"为什么"完全看不出来）。
    // 这里宁可报错：模板里没有这个变量 = 这个功能根本不可能工作。
    if (!str(tpl.content).includes(`{{${spec.varName}}}`)) {
      throw new Error(`提示词模板「${spec.template}」里没有 {{${spec.varName}}} 变量，卡片清单发不出去（请恢复该变量，或在设置里改回内置模板）`);
    }
    const r = await storyAsk(tpl, { [spec.varName]: story.fillLines(withText, spec) },
      { model: body.model, project_id: source.project_id, note: spec.promptNote });
    if (!r.ok) throw new Error(r.error);
    const byId = {};
    withText.forEach((e) => { byId[e.id] = e.text; });
    const plan = story.applyFillAssignments(withText, r.parsed, byId, spec);
    // 报告里记的必须是**真的写进去的东西**：值取截断后的（`clipField` 之后），引文原样带上。
    // 拿模型的提议去报告会重演补 32 那个假警报（按"模型说了什么"报异常），
    // 而且界面要让人一眼核对"写进去的值 ← 原文原话"，两边不是同一份数据就核对不了
    const written = [];
    for (const p of plan.patch) {
      const patch = {};
      // 照**计划里记下的字段清单**写，不要按 `kind` 再推一次：计划条目不是卡片、没有 `kind`，
      // 按 kind 推会得到空清单 —— 于是"接口报 assigned=2、卡片上却一个字都没有"（补 34 真踩到，
      // 这种"成功但没写"最难发现：状态码、计数、提示全都对）。清单由 applyFillAssignments 给出，同源
      for (const f of (p.fields || [])) {
        // 与"手改卡片"过**同一把尺子**（补 30）：模型写有上限，人写也得有，这里不能另开一条路
        if (p[f] != null) patch[f] = story.clipField(p[f], f);
      }
      if (Object.keys(patch).length) store.update('story_cards', p.id, patch);
      // 只报告真的落了库的那些（`patch` 为空 = 一个字都没写，不能算进 assigned）
      if (Object.keys(patch).length) {
        written.push({ id: p.id, name: p.name, index: p.index, quote: str(p.quote), fields: Object.keys(patch), values: patch });
      }
    }
    const noSource = targets.length - withText.length;
    return {
      ok: true, target: spec.key, assigned: written.length, targets: targets.length, pool: pool.length,
      calls: 1, with_source: withText.length, no_source: noSource, aligned,
      // 三种"没写成"的原因必须**分开报**：原文确实没写（not_found）、模型的话对不上原文（ungrounded）、
      // 没给全（missing）。合成一个数，用户就分不清"原著没写"和"模型在编"
      not_found: plan.not_found, ungrounded: plan.ungrounded, empty: plan.empty,
      missing: plan.missing, invalid: plan.invalid, no_id: plan.no_id,
      // 「写进去的值 + 它的引文」逐张回报 —— 引文是这个机制唯一可核对的东西，
      // 只报个张数（或一闪而过的 toast）等于让用户盲信；时间点这类**事实型**字段尤其如此
      assigned_items: written,
      assigned_names: written.map((x) => x.name),
      note: written.length ? '' : spec.noneNote,
    };
  });

  /**
   * AI 回原文补一张人物卡（批 8 补 43）。
   *
   * 关掉体检里**最后一条"只能人工"且 AI 确实做得了**的项：`plot_cast_unknown` 与它的镜头侧同族
   * `shot_char_unknown` —— 同一个病的两侧（有个名字，系统里没有这个人的档案）。原来体检只说
   * "去抽一张人物卡（或在角色库里建一个）"，可那件事**原著通常写了**，只是分块抽取那一步漏了。
   * 与补 33–35 的 field-fill 完全同一套纪律（干跑闸门 → 一次调用 → 引文核对 → 分桶上报），
   * 差别是它**新增数据**而不是就地修改，所以多一条红线：`not_found` 一个都不许建卡。
   */
  on('POST', '/api/story/cast-fill', async (req, res, params, body) => {
    const source = store.get('story_sources', str(body.source_id));
    if (!source) throw httpError(404, '原著不存在');
    const cards = store.list('story_cards', { filter: (r) => r.source_id === source.id });
    const projectChars = store.list('characters', { filter: (x) => x.project_id === source.project_id, limit: 100000 });
    // 候选名单与体检**同源**（`unknownPlotCast` 一份实现）：体检报几个，这里就是几个
    const { split, aligned } = reSplitSource(source);
    const targets = story.castFillTargets(cards, { characters: projectChars, chunks: split.chunks });
    const withText = targets.filter((t) => t.has_text);
    const base = {
      ok: true, assigned: 0, targets: targets.length, with_source: withText.length,
      no_source: targets.length - withText.length, calls: 0, aligned,
      not_found: [], ungrounded: [], empty: [], invalid: [], missing: [], assigned_items: [], assigned_names: [],
    };
    if (!cards.some((c) => c.kind === 'plot')) {
      throw httpError(400, '这份原著还没有剧情卡：先解析出剧情卡，才有「涉及人物」可以核对');
    }
    if (!targets.length) {
      return { ...base, note: '剧情卡里提到的名字都已经有人物卡或角色库档案了 —— 没有需要补的' };
    }
    // 干跑：先如实告诉用户"要确认几个名字、调几次"，确认了再花钱（与 field-fill 同一条闸门）
    if (bool(body.dry_run)) {
      return { ...base, dry_run: true, calls: 1,
        candidates: targets.map((t) => ({ name: t.name, cards: t.cards.length, has_text: t.has_text })) };
    }
    if (!withText.length) {
      return { ...base, note: '这些名字所在的剧情卡都没有可定位的原文出处（人工新建、或来自全局归并没有段号）——回原文找无从谈起，没有新建任何卡' };
    }
    const tpl = storyTemplate('cast_card');
    // 变量名对不上是**静默**失败（`renderPrompt` 把没给的 {{…}} 换成空串）：候选清单整个发不出去，
    // 模型只能凭名字编，而链路上零报错。宁可在这里报错 —— 模板里没有这个变量 = 这个功能不可能工作
    if (!str(tpl.content).includes('{{候选人物与原文片段}}')) {
      throw new Error('提示词模板「cast_card」里没有 {{候选人物与原文片段}} 变量，候选清单发不出去（请恢复该变量，或在设置里改回内置模板）');
    }
    const r = await storyAsk(tpl, { '候选人物与原文片段': story.castFillLines(withText) },
      { model: body.model, project_id: source.project_id, note: '原著解析 · 回原文补人物卡' });
    if (!r.ok) throw new Error(r.error);
    const plan = story.applyCastCards(withText, r.parsed);
    const byId = new Map(cards.map((c) => [c.id, c]));
    // `order` 排在最后：新卡不该插进已有卡的中间（顺序会进分集大纲与界面列表）
    const maxOrder = cards.reduce((m, c) => Math.max(m, Number(c.order) || 0), 0);
    const rows = [];
    const meta = [];
    plan.cards.forEach((p, i) => {
      // 落库走**唯一一份**规范化（`normalizeCard`）：与分块抽取抽出来的卡形状完全一致
      // （截断、别名清洗、字段白名单都在里面，这里绝不另开一条"自己拼一个卡对象"的路）
      const card = story.normalizeCard(
        { kind: 'character', name: p.name, ...p },
        { source_id: source.id, project_id: source.project_id, order: maxOrder + i + 1 },
      );
      if (!card) return; // 名字为空等 → normalizeCard 弃掉，如实少建一张（不猜）
      card.origin = 'cast_fill';
      // 这张卡是哪几张剧情卡逼出来的、依据在哪几段原文 —— 留痕，用户点开卡能回原文核对
      const src = withText[p.index - 1] || {};
      card.evidence = [...new Set((src.card_ids || [])
        .map((id) => byId.get(id))
        .flatMap((c) => (Array.isArray(c?.evidence) ? c.evidence : []))
        .filter((x) => Number.isInteger(x)))].sort((a, b) => a - b);
      card.mentions = 0;
      rows.push(card);
      // 报告条目**与 field-fill 同构**（name/fields/values/quote/card_ids）—— 界面因此能共用同一个
      // 报告渲染器（补 43 一开始给它单开了 created/created_names，那是同一个事实的第二份形状：
      // 两份形状迟早分叉，而分叉的表现是"报告里少了一列"，没人会注意到）
      meta.push({
        fields: p.fields,
        values: Object.fromEntries(p.fields.map((f) => [f, card[f]])), // 取 normalizeCard **之后**的值（真落库的那一份）
        quote: p.quote,
        card_ids: src.card_ids || [],
      });
    });
    // `insertMany` 回传的是**真的落库的那些行**（带 id）—— 报告要报"实际落库的那一份"，
    // 所以 id 必须取自它：`normalizeCard` 不产出 id，自己造一个就是报了一个不存在的 id
    const saved = rows.length ? store.insertMany('story_cards', rows) : [];
    const written = saved.map((c, i) => ({
      id: c.id, name: c.name, index: i + 1,
      fields: meta[i].fields, values: meta[i].values, quote: meta[i].quote, card_ids: meta[i].card_ids,
    }));
    return {
      ...base, assigned: saved.length, calls: 1,
      assigned_items: written, assigned_names: saved.map((c) => c.name),
      not_found: plan.not_found, ungrounded: plan.ungrounded, empty: plan.empty,
      invalid: plan.invalid, missing: plan.missing,
      note: rows.length ? '' : '模型没有给出可用的档案（原文里没写、或引文对不上原文）——没有新建任何卡',
    };
  });

  /**
   * AI 认名字（批 8 补 45）：把镜头「出场人物」里角色库**找不到**的名字，交给模型对着
   * **镜头自己的文字**与**项目角色名册**认一遍，认出来就**绑定**（不改用户写的字）。
   *
   * 为什么另立一个端点，而不是塞进 `field-fill` 的规格表：那三份规格的落点是**卡片上的字段**、
   * 候选来自**卡片库**；这一份的落点是**镜头上的绑定**、候选来自**体检**。落点与来源都不同，
   * 硬塞进去只会让 `FILL_SPECS` 里多一条"target 根本不是卡片"的异类（补 43 的同一条判断：
   * 该合并的是**实现**，不是端点）。
   *
   * 三条纪律与补 33–35/43 完全一致：**干跑闸门**（先报几个名字、影响几个镜头、调几次）、
   * **引文可验证**（`quoteGrounded` 唯一一份实现）、**分桶且互不重叠**（且每个桶都不是失败）。
   * 外加一条本端点的红线：**`target` 必须真的在角色名册里才绑** —— 绑定是按 id 的，
   * 模型给一个名册里没有的名字本来也绑不上，但"静默忽略"和"如实报 `unknown_target`"是两件事。
   */
  on('POST', '/api/story/shot-char-fill', async (req, res, params, body) => {
    const projectId = str(body.project_id).trim();
    ...
    const chars = store.list('characters', { filter: (r) => r.project_id === projectId, limit: 100000 });
    const cards = store.list('story_cards', { filter: (r) => r.project_id === projectId, limit: 100000 });
    // 候选**只来自体检自己报出来的那几条**（`auditShotBindings` 是唯一一份判据）：
    // 与用户眼睛看到的列表是同一份，于是"体检报 3 个、认名字只认 2 个"在结构上不可能（补 43 的教训）
    const aud = story.auditShotBindings(shots, { characters: chars, cards });
    const want = (Array.isArray(body.names) ? body.names : []).map((x) => str(x).trim()).filter(Boolean);
    const picked = aud.issues.filter((i) => i.code === 'shot_char_unknown'
      && (!want.length || want.includes(str(i.target_name))));
    const byId = new Map(shots.map((x) => [str(x.id), x]));
    const targets = story.shotCharTargets(picked, byId);
    const base = {
      ok: true, assigned: 0, targets: targets.length, shots: 0, calls: 0,
      bindings: [], passerby: [], need_card: [], ungrounded: [], unknown_target: [],
      invalid: [], empty: [], missing: [], assigned_items: [], assigned_names: [],
    };
    if (!picked.length) {
      // 400 只表示"这件事现在无从谈起"（与 field-fill/cast-fill 同口径）：一键补齐据此把它当**结论**、
      // 而不是当失败 —— 报成"这一步没做成"就是假警报，而假警报比不检查更糟
      throw httpError(400, want.length
        ? `体检里现在没有这几个名字了（可能已经被补上或绑定过了）：${want.join('、')}`
        : '所有「出场人物」都能在角色库里找到，没有要认的名字');
    }
    const withText = targets.filter((t) => t.has_text);
    // 干跑：先如实说"要认几个名字、影响几个镜头、调几次"，确认了再花钱
    if (bool(body.dry_run)) {
      return {
        ...base, dry_run: true, calls: 1,
        shots: withText.reduce((a, t) => a + t.shot_ids.length, 0),
        candidates: targets.map((t) => ({ name: t.name, shots: t.shot_ids.length, has_text: t.has_text })),
      };
    }
    if (!withText.length) {
      return { ...base, note: '这些镜头的「出场人物/画面描述/动作/台词」全是空的 —— 认名字无从谈起，没有绑定任何角色' };
    }
    const tpl = storyTemplate('shot_char');
    // 变量名对不上是**静默**失败：`renderPrompt` 会把"没给的 {{…}}"替换成**空串**，
    // 于是名册或候选清单整个发不出去、模型只能凭名字编，而链路上没有任何报错。这里宁可报错。
    for (const v of ['角色名册', '未知名字与镜头文字']) {
      if (!str(tpl.content).includes(`{{${v}}}`)) {
        throw new Error(`提示词模板「shot_char」里没有 {{${v}}} 变量，候选清单发不出去（请恢复该变量，或在设置里改回内置模板）`);
      }
    }
    // 名册走 `characterRoster` **唯一一份**（与生成剧本/分镜时发给模型的是同一段文字）：
    // 它本来就写着"原文里的代称要换成对应本名"，正是这一趟要用的判据 —— 另写一份提示词里的名册
    // 迟早会出现"生成时给的名册"与"认名字时给的名册"不一样（补 36/37 的教训）
    const roster = story.characterRoster(chars, { limit: 60 });
    const r = await storyAsk(tpl, {
      '角色名册': roster.text || '（项目里还没有角色档案 —— 那么每个名字都只能报 need_card 或 passerby）',
      '未知名字与镜头文字': story.shotCharLines(withText),
    }, { model: body.model, project_id: projectId, note: '分镜 · AI 认名字' });
    if (!r.ok) throw new Error(r.error);
    const plan = story.applyShotCharBindings(withText, r.parsed, { characters: chars });
    // 落库：**两处一起写**（只写一处就会留下一条永久假警报，见 `shotCharTargets` 上的说明）。
    // ① 绑定走**并集**语义（与补 5 的自动匹配、体检的 `bind_shot_target` 共用同一个落点与同一句话），
    //    绝不覆盖人工绑定；② 把「出场人物」里的那个代称换成本名 —— 体检判的是**这段文本**，
    //    只绑不改的话 `shot_char_unknown` 永远消不掉（而它宣称的后果其实已经修好了）。
    // 报告只记**真的写了**的那些 —— 一个镜头都没写成的不能算进 assigned
    const shotById = new Map(shots.map((x) => [str(x.id), x]));
    const written = [];
    let boundShots = 0;
    for (const b of plan.bindings) {
      let n = 0;
      for (const sid of b.shot_ids) {
        const sb = shotById.get(str(sid));
        if (!sb || str(sb.project_id) !== projectId) continue;
        const cur = Array.isArray(sb.character_ids) ? sb.character_ids.map(String) : [];
        const patch = {};
        if (!cur.includes(str(b.character_id))) patch.character_ids = cur.concat(str(b.character_id));
        // 改名只在**真的提到过这个代称**时做（候选的名字就是从这段文本里切出来的，所以通常一定命中；
        // 但同一次调用里前一个名字可能已经把这段文本改过了 —— 那时改名是空操作，如实跳过）
        const renamed = story.renameShotCharacter(sb.characters, b.name, b.character_name);
        if (renamed !== str(sb.characters)) patch.characters = renamed;
        if (!Object.keys(patch).length) continue;
        store.update('storyboards', str(sid), patch);
        n++;
      }
      if (!n) continue;
      boundShots += n;
      written.push({ name: b.name, index: b.index, quote: b.quote, shot_ids: b.shot_ids, bound: n,
        character_id: b.character_id, character_name: b.character_name });
    }
    return {
      ...base, assigned: written.length, shots: boundShots, calls: 1,
      // 报告条目与 field-fill/cast-fill **同构**（name/fields/values/quote）—— 界面因此能共用
      // 同一个报告渲染器。`values` 取**真的写进去的那一份**：这里写进去的是两件事
      // （「出场人物」的代称换成了本名 + 绑上了这个角色），两件都列出来 —— 写了两处却只报一处，
      // 就是"只被显示、没被读取"的反面：**写了却没被显示**，用户核对不到
      assigned_items: written.map((w) => ({
        name: w.name, fields: ['出场人物', '绑定角色'],
        values: { 出场人物: w.character_name, 绑定角色: w.character_name }, quote: w.quote,
      })),
      assigned_names: written.map((w) => w.name),
      passerby: plan.passerby, need_card: plan.need_card, ungrounded: plan.ungrounded,
      unknown_target: plan.unknown_target, invalid: plan.invalid, empty: plan.empty, missing: plan.missing,
      note: written.length ? '' : '模型没有认出任何一个（名册里没有对应的人、或引文对不上镜头文字）——没有绑定任何角色',
    };
  });
  ...
  on('GET', '/api/story/episode-brief', (req, res, params, body, query) => {
    ...
    const priorMax = num(query.prior_max, story.PRIOR_MAX_DEFAULT);
    // 批 8 补 37：名册必须**挂上 plan**（`planWithRosters`）—— 指纹在"存"（这里）与"复算"
    // （`/api/story/staleness`）两处各算一次，任何一处少挂名册，那处算出来的就是另一个指纹，
    // 结果是**每一集都永久报过期**。合成一步，漏挂就变成不可能。
    const plan = story.planWithRosters(cards,
      store.list('characters', { filter: (r) => (projectId ? r.project_id === projectId : true), limit: 100000 }),
      { perEpisode: num(query.per_episode, story.EPISODE_PER_DEFAULT), priorMax });
    const ep = Math.max(1, num(query.episode, 1));
    const brief = story.episodeBriefText(plan, ep);
    const prior = story.priorBrief(plan, ep, { maxChars: priorMax });
    const withPrior = query.with_prior !== '0';
    const roster = (plan.roster_texts || {})[ep] || { count: 0, names: [], text: '', truncated: 0 };
    return {
      ok: true,
      episode_number: ep,
      // 批 8 补 12：生成这一集时"喂给模型的输入"的指纹。前端存剧本时原样带上 ——
      // 指纹算法只在这里实现一份（前端复算一遍迟早会跟服务端漂移）
      input_digest: story.episodeInputDigest(plan, ep, { withPrior, priorMax }),
      // 名册也要能"如实说出带了什么"：带没带在界面上长得一样就白带了（补 36 的教训）
      roster_text: roster.text,
      roster_count: roster.count,
      roster_truncated: roster.truncated,
      prior_chars: prior.chars,
      episode_count: plan.episodes.length,
      per_episode: plan.per_episode,
      exists: !!brief,
      brief,
      // 批 8 补 36：本集拍表现在**也带全剧设定/时间线**（从前只有"全剧大纲"那条路有，
      // 逐集生成看不见 —— 跨集设定漂移就是这么来的）。这三个字段让界面能如实说出
      // "这份上下文里带了什么、有多大"，而不是让用户猜。设定块**不截断**：
      // 它是跨集一致性的锚点，悄悄截一半比不带更糟（要报就报全）。
      setting_chars: str(plan.setting_text).length,
      world_count: plan.world_count,
      timeline_count: plan.timeline_count,
      prior: prior.text,
  ...
  // 角色名册（批 8 补 37）：**纯计算、不写任何东西**，只是把名册渲染放在服务端一处。
  // 为什么用 POST 而不是 GET：要渲染的文本（整集大纲 + 前情）动辄几千字，塞进 query 会撞 414。
  // 为什么前端不自己算：名册是喂给模型的输入，而"输入变了要报过期"靠服务端算的指纹 ——
  // 两处各写一遍排序/截断，迟早出现"提示词里的名册与指纹里的名册不是同一份"。
  on('POST', '/api/story/roster', (req, res, params, body) => {
    const projectId = str(body.project_id).trim();
    const chars = store.list('characters', {
      filter: (r) => (projectId ? r.project_id === projectId : true),
      limit: 100000,
    });
    const r = story.characterRoster(chars, { text: str(body.text), limit: num(body.limit, 30) });
    return { ok: true, count: r.count, names: r.names, text: r.text, truncated: r.truncated };
  });
  ...
  on('GET', '/api/story/staleness', (req, res, params, body, query) => {
    ...
    // 与 `/api/story/episode-brief` **必须**用同一个构造函数：指纹要在两边算出一模一样的结果，
    // 少挂名册的那一边会把每一集都判成过期（假警报比不检查更糟）
    const plan = story.planWithRosters(cards,
      store.list('characters', { filter: (r) => (projectId ? r.project_id === projectId : true), limit: 100000 }),
      { perEpisode: num(query.per_episode, story.EPISODE_PER_DEFAULT), priorMax: num(query.prior_max, story.PRIOR_MAX_DEFAULT) });
    ...
    const r = story.auditStaleness(plan, scripts, shots, {
      ...
      // 批 8 补 38：分镜指纹要复算"名册 + 正文"，所以必须把角色一并给它 ——
      // 与入库那一刻 `sourceDigest` 用的那一份同源（同一个构造函数、同一个项目范围）
  ...
  on('POST', '/api/story/cards/:id/to-character', (req, res, params, body) => {
    ...
    const character = insertCharacter(story.cardToCharacter({ ...card, project_id: projectId }, projectId));
  ...
  on('POST', '/api/story/cards/import-characters', (req, res, params, body) => {
    ...
    for (const card of all) {
      ...
      const c = insertCharacter(story.cardToCharacter({ ...card, project_id: projectId }, projectId));
  ...
  on('GET', '/api/story/audit', (req, res, params, body, query) => {
    ...
    // 批 8 补 40：润色稿的派生过期要同时看**来源剧本**（可能在别处）与**模板**，所以剧本要全量取，
    // 不能只取当前项目的 —— 来源剧本换过项目的话，只取本项目就会把"来源不见了"误报成"来源被删了"
    const scriptRows = store.list('scripts', { limit: 100000 });
    ...
    // 批 8 补 39：提示词是**派生字段**（画面描述 + 模板 → 英文提示词，原地写回同一行）。
    // 改了描述之后提示词还是旧的，而链路上没有任何一层会报 —— 它不是"该重生成分镜"，
    // 是"这一行的这个派生字段过期了"。判定纯本地，与体检同一条纪律：随时可跑、不花钱。
    const promptStale = story.auditPromptStaleness(shotRows, {
      characters: projectChars,
      templates: { image: tryPromptTemplate('image'), video: tryPromptTemplate('video') },
    });
    // 批 8 补 40：润色是**另存一条**（不是原地写回），所以它的过期是"**这条派生记录的来源变了**"。
    // 与补 39 的提示词是同一种病的两种形态：那一条的症状是"改了描述、提示词没变"，
    // 这一条是"改了来源剧本/优化模板/角色外貌、润色稿没变"，而它还带着 plan_digest 报到"没过期"。
    // 模板列表是**同一次判定里的同一个事实**：两条判定（润色派生 / 生成模板）都按 id 找模板，
    // 取两份会让"改了一处忘了另一处"这种错有机会发生（补 36/37 的教训）
    const allTpls = store.list('prompt_templates', { limit: 100000 });
    const polishStale = story.auditPolishStaleness(scriptRows, {
      characters: projectChars,
      templates: allTpls,
    });
    // 批 8 补 41：**生成模板也是输入**。补 12/36/37 建的是剧本的"数据侧"指纹，
    // 而真正发出去的 system + content 来自设置页那个可编辑的模板 —— 改了模板，旧剧本照旧报 ok。
    const tplStale = story.auditScriptTemplate(scriptRows, { templates: allTpls });
    const counts = {
      warn: r.counts.warn + shots.counts.warn + styleIssues.counts.warn + drift.counts.warn + refGaps.counts.warn + cast.counts.warn + promptStale.counts.warn + polishStale.counts.warn + tplStale.counts.warn,
      info: r.counts.info + shots.counts.info + styleIssues.counts.info + drift.counts.info + refGaps.counts.info + cast.counts.info + promptStale.counts.info + polishStale.counts.info + tplStale.counts.info,
      fixable: r.counts.fixable + shots.counts.fixable + styleIssues.counts.fixable + drift.counts.fixable + refGaps.counts.fixable + cast.counts.fixable + promptStale.counts.fixable + polishStale.counts.fixable + tplStale.counts.fixable,
    };
    return {
      ...
      ref_counts: refGaps.counts, cast_counts: cast.counts, prompt_counts: promptStale.counts,
      polish_counts: polishStale.counts, polish_derived: polishStale.derived,
      template_counts: tplStale.counts, template_stamped: tplStale.stamped,
      prompt_by_field: promptStale.by_field,
      drift_pairs: drift.pairs,
      issues: r.issues.concat(shots.issues, styleIssues.issues, drift.issues, refGaps.issues, cast.issues, promptStale.issues, polishStale.issues, tplStale.issues),
      card_issues: r.issues, shot_issues: shots.issues, style_issues: styleIssues.issues, drift_issues: drift.issues,
      ref_issues: refGaps.issues, cast_issues: cast.issues, prompt_issues: promptStale.issues,
      polish_issues: polishStale.issues, template_issues: tplStale.issues,
    };
  });
  ...
  on('POST', '/api/story/audit/fix', (req, res, params, body) => {
    ...
    if (code === 'lock_shot_char') {
      ...
      updateCharacter(targetId, { is_locked: true });
    ...
    if (code === 'sync_character') {
      ...
      // 上限取自同一张表（这里以前写死 400，与 cardToCharacter 的 400、表里的数各是一份）
      patch.notes = story.clipCharacterField(
        `${str(ch.notes).trim() ? `${str(ch.notes).trim()}；` : ''}与人物卡同步（${fields.join('/')}）`, 'notes',
      );
