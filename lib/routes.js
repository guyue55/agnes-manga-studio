...
module.exports = function createRoutes(ctx) {
  ...
  on('GET', '/api/scripts', (req, res, params, body, query) => {
    ...
    const audit = story.auditPolishStaleness(store.list('scripts', { limit: 100000 }), {
      characters: projectId ? projectCharacters(projectId) : store.list('characters', { limit: 100000 }),
      templates: store.list('prompt_templates', { limit: 100000 }),
    });
    return rows.map((r) => Object.assign({}, r, { polish_state: audit.states[r.id] || '' }));
  });
  ...
  on('GET', '/api/story/audit', (req, res, params, body, query) => {
    ...
    const counts = {
      warn: r.counts.warn + shots.counts.warn + styleIssues.counts.warn + drift.counts.warn + refGaps.counts.warn + cast.counts.warn + promptStale.counts.warn + polishStale.counts.warn,
      info: r.counts.info + shots.counts.info + styleIssues.counts.info + drift.counts.info + refGaps.counts.info + cast.counts.info + promptStale.counts.info + polishStale.counts.info,
      fixable: r.counts.fixable + shots.counts.fixable + styleIssues.counts.fixable + drift.counts.fixable + refGaps.counts.fixable + cast.counts.fixable + promptStale.counts.fixable + polishStale.counts.fixable,
    };
    return {
      ...
      issues: r.issues.concat(shots.issues, styleIssues.issues, drift.issues, refGaps.issues, cast.issues, promptStale.issues, polishStale.issues),
      ...
      polish_issues: polishStale.issues,
    };
  });
