...
module.exports = function createRoutes(ctx) {
  ...
  on('POST', '/api/agnes/image', async (req, res, params, body) => {
    ...
    const sbRow = body.storyboard_id ? (store.get('storyboards', str(body.storyboard_id)) || {}) : {};
    ...
    const inputImages = [...new Set([...explicit, ...ref.urls])].slice(0, 4);
    ...
    try {
      ...
      store.insert('generation_tasks', {
        ...
        task_type: 'image',
        model_name: model,
        input_content: { prompt, size, image: inputImages[0] || null, reference_images: ref.urls },
      ...
      return {
        ...
        reference_images: {
          used: ref.urls.length, characters: ref.chars,
          local_skipped: ref.local, missing: ref.missing,
          attached: inputImages.length,
        },
      };
    } catch (e) {
  ...
  on('GET', '/api/story/audit', (req, res, params, body, query) => {
    ...
    const counts = {
      warn: r.counts.warn + shots.counts.warn + styleIssues.counts.warn + drift.counts.warn,
      info: r.counts.info + shots.counts.info + styleIssues.counts.info + drift.counts.info,
      fixable: r.counts.fixable + shots.counts.fixable + styleIssues.counts.fixable + drift.counts.fixable,
    };
    return {
      ...
      issues: r.issues.concat(shots.issues, styleIssues.issues, drift.issues),
