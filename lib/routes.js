...
module.exports = function createRoutes(ctx) {
  ...
  function reSplitSource(source) {
    ...
    return { split, aligned: split.chunks.length === num(source?.chunk_count, 0) };
  }
  ...
  on('GET', '/api/story/coverage', (req, res, params, body, query) => {
    ...
    // 只给"第 7 段"等于让他自己去数
    const { split: chunks, aligned } = reSplitSource(source);
    ...
    return {
      ...
      // 重新切块切不出原样的段数：预览可能对不上原文，**如实说**，别让用户看着错位的预览做判断
      aligned,
      notes: [
        ...(sum.has_states ? [] : ['这份原著是本轮之前解析的，没有逐块记录：只能看出"哪些段没有卡片"，不能判断是没信息还是漏了']),
        ...(aligned ? [] : [`重新切块得到 ${chunks.chunks.length} 段，与解析时的 ${chunkCount} 段不一致，预览仅供参考`]),
      ],
    };
  });
   ...
   * 所以"补抽"永远不会把用户已经改好的卡片冲掉，也不会让分镜绑定悬空。
   */
  on('POST', '/api/story/retry-chunks', (req, res, params, body) => {
    ...
    const { split: all, aligned } = reSplitSource(source);
    if (!all.chunks.length) throw httpError(400, '这份原著里没有可解析的文字');
    // 切不回原样的段数就**不要动手**：块号已经对不上原文了，硬跑会把别的段落当成第 N 段重抽，
    // 用户看到"补抽成功"却得到一段完全无关的卡片 —— 宁可报错让人去重解析
    if (!aligned) {
  ...
  on('GET', '/api/story/card-source', (req, res, params, body, query) => {
    ...
    if (!aligned) notes.push('这份原著重新切块得到的段数与解析时不一致，下面的原文可能对不上这张卡（建议对这份原著重新解析）');
    ...
    return { ...base, source_id: source.id, source_title: str(source.title), chunk_count: split.chunks.length, excerpts, aligned, notes };
  });
  ...
  on('GET', '/api/story/chapters', (req, res, params, body, query) => {
    ...
    // 卡片按**出现过的章**计数：一张卡跨两章就算在两章里（"这章有没有内容"问的是出现，不是归属）
    ...
    for (const c of cards) {
      ...
      for (const i of idxs) {
        ...
        if (ch >= 0 && !seen.has(ch)) { seen.add(ch); cardsOf.set(ch, (cardsOf.get(ch) || 0) + 1); }
      }
    }
  ...
  const STORY_CARD_EDITABLE = ['name', 'summary', 'aliases', 'identity', 'appearance', 'outfit',
    'personality', 'role', 'gender', 'age', 'atmosphere', 'region', 'time_of_day', 'features',
    'owner', 'usage', 'stage', 'conflict', 'turn', 'outcome', 'involved', 'when', 'order_note',
    'genre', 'tone', 'worldview', 'theme', 'logline', 'mainline',
  ...
  on('GET', '/api/story/audit', (req, res, params, body, query) => {
    ...
    const counts = {
      warn: r.counts.warn + shots.counts.warn + styleIssues.counts.warn + drift.counts.warn + refGaps.counts.warn,
      info: r.counts.info + shots.counts.info + styleIssues.counts.info + drift.counts.info + refGaps.counts.info,
      fixable: r.counts.fixable + shots.counts.fixable + styleIssues.counts.fixable + drift.counts.fixable + refGaps.counts.fixable,
    };
    return {
      ...
      ref_counts: refGaps.counts,
      drift_pairs: drift.pairs,
      issues: r.issues.concat(shots.issues, styleIssues.issues, drift.issues, refGaps.issues),
      card_issues: r.issues, shot_issues: shots.issues, style_issues: styleIssues.issues, drift_issues: drift.issues,
      ref_issues: refGaps.issues,
    };
  });
