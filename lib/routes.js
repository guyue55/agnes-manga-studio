...
module.exports = function createRoutes(ctx) {
  ...
  on('GET', '/api/projects/:id/export.csv', (req, res, params, body, query) => {
    ...
    const head = ['集数', '镜头号', '景别', '画面描述', '人物', '绑定角色', '台词', '时长秒', '状态', '图片提示词', '图片提示词·最终词（含角色与画风）', '视频提示词', '视频提示词·最终词（文生视频口径；图生/多帧模式实际不注入）'];
    const body1 = rows.map((r) => [
      r.episode_number, r.shot_number, r.shot_type, r.scene_description, r.characters,
      resolveChars(r.character_ids).map((c) => c.name).join('、'), r.dialogue,
      r.duration_seconds, SB_STATUS_ZH[r.status] || r.status,
      r.image_prompt, finalPrompt(r.image_prompt, { projectId: params.id, storyboardId: r.id }),
      r.video_prompt, finalPrompt(r.video_prompt, { projectId: params.id, storyboardId: r.id }),
    ].map(csvCell).join(','));
  ...
  on('GET', '/api/projects/:id/export.md', (req, res, params, body, query) => {
    ...
    const L = [
      ...
      '- 下方提示词已含出场角色与画风注入；**图生/多帧视频**实际发送时不注入（长相与画风由参考图决定）', '',
    ];
    let curEp = null;
    for (const r of rows) {
      if (ep == null && r.episode_number !== curEp) { curEp = r.episode_number; L.push(`\n## 第 ${r.episode_number} 集`, ''); }
      L.push(`### #${r.shot_number} ${r.shot_type || ''} · ${r.duration_seconds || '?'}s`,
        r.scene_description ? `> ${r.scene_description}` : '',
        r.dialogue ? `台词：${r.dialogue}` : '',
        r.image_prompt ? `\n**图片提示词**（生成时追加出场角色与画风）\n\n\`\`\`\n${finalPrompt(r.image_prompt, { projectId: params.id, storyboardId: r.id })}\n\`\`\`` : '',
        r.video_prompt ? `\n**视频提示词**（生成时追加出场角色与画风）\n\n\`\`\`\n${finalPrompt(r.video_prompt, { projectId: params.id, storyboardId: r.id })}\n\`\`\`` : '', '');
    }
  ...
  on('POST', '/api/videos', async (req, res, params, body) => {
    // 图生/多帧模式的画风与长相都由参考图自身携带，只给纯文生视频补画风与角色短语。
    // （与既有画风规则同一判据：文字描述与参考图打架时，模型会给出更差的画面）
    const vMode = str(body.mode || 'text_to_video');
    const prompt = vMode === 'text_to_video'
      ? finalPrompt(body.prompt, { projectId: body.project_id, characterIds: body.character_ids, storyboardId: body.storyboard_id })
  ...
  function finalPrompt(base, opts = {}) {
    ...
    return artStylePhrase(characterPhrase(text, chars), opts.projectId ? styleOf(opts.projectId) : '');
  }

  on('POST', '/api/agnes/image', async (req, res, params, body) => {
    // 使用点注入：角色 + 画风都只在这里拼，入库的 image_prompt 永远只存镜头内容
    const prompt = finalPrompt(body.prompt, { projectId: body.project_id, characterIds: body.character_ids, storyboardId: body.storyboard_id });
