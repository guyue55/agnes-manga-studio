...
module.exports = function createRoutes(ctx) {
  ...
  on('GET', '/api/projects/:id/export.csv', (req, res, params, body, query) => {
    ...
    const head = ['集数', '镜头号', '景别', '画面描述', '人物', '台词', '时长秒', '状态', '图片提示词', '图片提示词·含画风', '视频提示词', '视频提示词·含画风'];
    const body1 = rows.map((r) => [
      r.episode_number, r.shot_number, r.shot_type, r.scene_description, r.characters, r.dialogue,
      r.duration_seconds, SB_STATUS_ZH[r.status] || r.status,
      r.image_prompt, artStylePhrase(r.image_prompt || '', proj.art_style),
      r.video_prompt, artStylePhrase(r.video_prompt || '', proj.art_style),
    ].map(csvCell).join(','));
  ...
  on('GET', '/api/projects/:id/export.md', (req, res, params, body, query) => {
    ...
    const ep = query.episode ? Number(query.episode) : null;
    const rows = store.list('storyboards', { filter: (r) => r.project_id === params.id && (ep ? Number(r.episode_number) === ep : true), limit: 100000, sort: (a, b) => (Number(a.episode_number) - Number(b.episode_number)) || (Number(a.sort_order) - Number(b.sort_order)) || (Number(a.shot_number) - Number(b.shot_number)) });
    const L = [`# ${proj.name || '项目'} · 镜头提示词导出`, '', `- 画幅：${proj.aspect_ratio || '默认'}｜画风：${proj.art_style || '（未设，提示词即最终词）'}`, ''];
    let curEp = null;
    for (const r of rows) {
      ...
      L.push(`### #${r.shot_number} ${r.shot_type || ''} · ${r.duration_seconds || '?'}s`,
        ...
        r.image_prompt ? `\n**图片提示词**（生成时追加画风）\n\n\`\`\`\n${artStylePhrase(r.image_prompt, proj.art_style)}\n\`\`\`` : '',
        r.video_prompt ? `\n**视频提示词**\n\n\`\`\`\n${artStylePhrase(r.video_prompt, proj.art_style)}\n\`\`\`` : '', '');
    }
