...
module.exports = function createRoutes(ctx) {
  ...
  on('GET', '/api/story/audit', (req, res, params, body, query) => {
    ...
    return { ok: true, total_cards: cards.length, counts: r.counts, issues: r.issues };
  });
  ...
  on('POST', '/api/story/audit/fix', (req, res, params, body) => {
    ...
    throw httpError(400, `不认识的体检项「${code}」（只支持 dup_name / alias_collision / char_not_in_asset）`);
  });
