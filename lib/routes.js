...
module.exports = function createRoutes(ctx) {
  ...
  on('POST', '/api/story/field-fill', async (req, res, params, body) => {
    ...
    const empty = { ok: true, target: spec.key, assigned: 0, pool: pool.length, targets: targets.length,
      ...
      empty: [], missing: [], invalid: [], no_id: 0, assigned_names: [] };
    if (!pool.length) {
    ...
    return {
      ok: true, target: spec.key, assigned: plan.patch.length, targets: targets.length, pool: pool.length,
      ...
      assigned_names: plan.patch.map((x) => x.name),
      note: plan.patch.length ? '' : spec.noneNote,
    };
  });
