...
module.exports = function createRoutes(ctx) {
  ...
  on('GET', '/api/story/audit', (req, res, params, body, query) => {
    ...
    const shots = story.auditShotBindings(
      store.list('storyboards', { filter: (x) => (projectId ? x.project_id === projectId : true), limit: 100000 }),
      {
        characters: store.list('characters', { filter: (x) => (projectId ? x.project_id === projectId : true), limit: 100000 }),
        cards,
      },
    );
    const counts = { warn: r.counts.warn + shots.counts.warn, info: r.counts.info + shots.counts.info, fixable: r.counts.fixable + shots.counts.fixable };
    return {
      ok: true, total_cards: cards.length, shots_scanned: shots.scanned,
      counts,
      card_counts: r.counts, shot_counts: shots.counts,
      issues: r.issues.concat(shots.issues),
      card_issues: r.issues, shot_issues: shots.issues,
    };
  });
  ...
  on('POST', '/api/story/audit/fix', (req, res, params, body) => {
    ...
    throw httpError(400, `不认识的体检项「${code}」（只支持 dup_name / alias_collision / char_not_in_asset / bind_shot_target / lock_shot_char）`);
  });
