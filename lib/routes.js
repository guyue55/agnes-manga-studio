...
module.exports = function createRoutes(ctx) {
  ...
  on('DELETE', '/api/videos/:id', (req, res, params) => {
    ...
    // R7：删掉已关联的视频时清掉镜头回链，避免分镜表指向不存在的任务
    if (a.storyboard_id) {
      const sb = store.get('storyboards', a.storyboard_id);
      if (sb && sb.linked_video_id === a.id) store.update('storyboards', a.storyboard_id, { linked_video_id: null });
    }
    if (a.local_file && fs.existsSync(a.local_file)) {
