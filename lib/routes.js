...
module.exports = function createRoutes(ctx) {
  ...
  on('POST', '/api/story/analyze', (req, res, params, body) => {
    ...
    (async () => {
      await jobs.run(job, items, async (item) => {
        ...
        if (!ex.cards.length) return { ok: false, error: '本段没有抽到任何卡片（原文可能无剧情信息，或模型未按格式返回）' };
      ...
      const failedChunks = job.items.filter((it) => it.state === 'fail').length;
  ...
  async function runStoryReduce(source, cards, opts = {}) {
    ...
    store.removeWhere('story_cards', (r2) => r2.source_id === source.id && r2.origin === 'bible');
    store.insertMany('story_cards', rows);
  ...
  on('GET', '/api/story/audit', (req, res, params, body, query) => {
    ...
    const counts = {
      warn: r.counts.warn + shots.counts.warn + styleIssues.counts.warn,
      info: r.counts.info + shots.counts.info + styleIssues.counts.info,
      fixable: r.counts.fixable + shots.counts.fixable + styleIssues.counts.fixable,
    };
    return {
      ok: true, total_cards: cards.length, shots_scanned: shots.scanned,
      art_style: style,
      counts,
      card_counts: r.counts, shot_counts: shots.counts, style_counts: styleIssues.counts,
      issues: r.issues.concat(shots.issues, styleIssues.issues),
      card_issues: r.issues, shot_issues: shots.issues, style_issues: styleIssues.issues,
    };
  });
  ...
  on('POST', '/api/story/audit/fix', (req, res, params, body) => {
    ...
    throw httpError(400, `不认识的体检项「${code}」（只支持 dup_name / alias_collision / char_not_in_asset / bind_shot_target / lock_shot_char / strip_style_word）`);
  });
