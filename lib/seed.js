...
const DEFAULT_TEMPLATES = [
  ...
  },
  {
    ...
    builtin_version: 1,
    name: '分镜 · AI 认名字',
    template_type: 'shot_char',
    system: '你是资深的漫剧编剧，但这一趟**只做一件事**：判断镜头「出场人物」里那个名字，指的是**名册里的哪一个已有角色**。你不创作、不新增角色、不改名字、不替用户写设定。名册里没有这个人时，如实说"需要先建档"或"是路人"，不要硬套一个最像的角色。严格按要求的 JSON 返回，不要输出 JSON 以外的任何文字、解释或代码块标记。',
    content: [
      '下面是**待确认的名字清单**，每个名字都附上了**提到它的那些镜头自己的文字**（出场人物/画面描述/动作/台词等）。',
      '这些名字写在镜头的「出场人物」里，但项目角色库里没有这个名字 —— 于是这些镜头**不会有外貌注入**，',
      '同一个角色在不同镜头里会换脸，而且链路上不会有任何报错。',
      '',
      '请对着【镜头文字】与【本剧角色名册】判断每一个名字指的是谁。四条硬要求：',
      '1. **必须给出镜头文字里的原话**（quote 字段）作为依据 —— 一字不改地照抄那几个字。',
      '   我们会把 quote 与镜头文字逐字比对：**对不上的一律不绑**，所以不要改写、不要拼接、不要用同义词替换。',
      '2. `target` 只能填**名册里出现过的名字**（本名或别名，一个字都不要改）。',
      '   名册里没有这个人时**不要填 target**，改用 `why` 如实说明是哪一种：',
      '   · `need_card` —— 原著里确实有这个人，但项目里还没有他的档案（需要先建卡/建档）；',
      '   · `passerby` —— 路人、临时角色、群演（"丫鬟""侍卫甲"这类，不值得为它建档）。',
      '   这两种都是**正确的答案，不是失败**：宁可不绑，也不要把一个角色硬套到另一个人身上 ——',
      '   绑错了会让这个镜头注入**别人的脸**，比不注入更难发现。',
      '3. **认不出来就说认不出来**：镜头文字里没有任何能指向某个名册角色的线索时，填 `why=passerby`，',
      '   不要因为"名册里只有她一个女的"就认定是她。',
      '4. 一个名字只回一条；`index` 用上面给的编号。',
      '',
      '返回 JSON（index 用上面给的编号）：',
      '{"names":[{"index":1,"target":"名册里的本名或别名","quote":"镜头文字里的原话"},{"index":2,"target":"","why":"need_card","quote":"镜头文字里的原话"},{"index":3,"target":"","why":"passerby","quote":"镜头文字里的原话"}]}',
      '',
      '【本剧角色名册】',
      '{{角色名册}}',
      '',
    ...
    negative_prompt: '',
    notes: '分镜 · 把镜头「出场人物」里角色库找不到的名字认一遍，认出来就**绑定**（不改用户的文本），一次调用；引文对不上就丢弃、不绑',
  },
  {
    key: 'plot_stage',
    builtin_version: 1,
