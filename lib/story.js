...
/**
 * "这张人物卡缺长相"的**唯一判据**（批 8 补 33）。
 *
 * 体检的 `char_no_look` 与"AI 回原文补长相"的候选名单**必须同源** —— 手抄第二遍的话，
 * 某天把判据从"两者都空"改成"外貌为空"，两处就会各说各话（对照补 30 的教训）。
 */
function needsLook(c) {
  return !!c && c.kind === 'character'
    && !str(c.appearance).trim() && !str(c.outfit).trim();
}

/** 补长相的候选名单：只挑缺长相的人物卡（与体检同一条判据）。 */
function lookFillTargets(cards) {
  const list = (Array.isArray(cards) ? cards : []).filter(Boolean);
  const chars = list.filter((c) => c.kind === 'character');
  return { chars, targets: chars.filter(needsLook) };
}

/**
 * 给每张候选卡取"它自己那段原文"（批 8 补 33）。
 *
 * **为什么必须回原文**：人物的长相在原著里通常**写了**，只是抽取时漏了 —— 那么"回去找一遍"是
 * **抽取**，不是创作。让模型凭名字编一个长相，会变成"看起来补上了、其实是我们编的"，
 * 而且它会一路进每一次出图提示词，且**与写对的长相在界面上长得一模一样**。
 *
 * 纯函数：块文本由调用方给（切块逻辑在 routes.js 的 reSplitSource，本层不碰 IO）。
 * 只取前 `maxChunks` 段、总长 `maxChars` —— 一张卡的出处可能有七八段，全塞进提示词既贵又稀释重点。
 */
function lookFillExcerpts(targets, chunkTexts, opts = {}) {
 ...
 */
function lookFillLines(entries) {
...
function applyLookAssignments(targets, payload, excerptById) {
  ...
  for (const raw of looks) {
    ...
    if (!Number.isInteger(n) || n < 1 || n > list.length) {
      invalid.push({ index: raw?.index, reason: 'index_out_of_range' });
      continue;
    }
    ...
    if (raw?.found === false || raw?.found === 'false') {
    ...
    const quote = norm(raw?.quote);
    if (!quote || !excerpt || !excerpt.includes(quote)) {
      ungrounded.push({ id: str(c.id), index: n, name: str(c.name), quote: str(raw?.quote).slice(0, 40) });
      continue;
    }
    const one = {};
    for (const f of ['appearance', 'outfit']) {
      const v = str(raw?.[f]).trim();
      if (!v) continue;
      one[f] = v; // 长度由调用方过 `clipField`（与手改同一条尺子）——本层只判定"接不接住"
    }
    if (!Object.keys(one).length) {
      empty.push({ id: str(c.id), index: n, name: str(c.name) });
      continue;
    }
    patch.push({ id: str(c.id), index: n, name: str(c.name), quote: str(raw?.quote), ...one });
  }
  ...
  return { patch, not_found, ungrounded, empty, invalid, missing, no_id, targets: list.length, looks: looks.length };
}
...
function auditCards(cards, opts = {}) {
  ...
  for (const c of list) {
    ...
    push({
      ...
      go: {
        page: 'novel',
        params: { project_id: c.project_id || '', source_id: c.source_id || '', kind: 'character' },
      },
    });
  }
