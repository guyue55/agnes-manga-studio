...
function applyFillAssignments(targets, payload, excerptById, spec) {
  ...
  for (const item of raw) {
    ...
    const excerpt = norm(excerptById?.[str(c.id)] ?? excerptById?.[n] ?? '');
    const quote = norm(item?.quote);
    if (!quote || !excerpt || !excerpt.includes(quote)) {
...
function auditPlotCast(cards, opts = {}) {
  ...
  for (const c of list) {
    ...
    for (const nm of splitShotCharacters(c.involved)) {
      ...
      // 词里含已知名字 → 视为已知（吸收"顾寒的对峙"这类自由写法，见函数头注释）
  ...
  const issues = [...groups.values()].map((g) => {
    ...
    return {
      ...
      detail: `这 ${n} 张剧情卡的「涉及人物」写了「${g.name}」，但整个项目里没有这个名字的人物卡，角色库里也没有。`
        ...
        + `若他确实是新角色，去抽一张人物卡（或在角色库里建一个），这条就会消失；若只是"路人甲"这种泛称，可以忽略。`,
