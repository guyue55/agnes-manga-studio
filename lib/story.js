  ...
  for (const c of list) {
    ...
    push({
      ...
      detail: '补上「第三天黄昏」这类时间锚点，跨集的时间推进才不会自相矛盾。'
        // 这一项**故意不做 AI 补值**：时间点是"关于世界的事实"，原文没写就只能猜，
        // 而猜出来的时间线会当正史进剧本提示词、且与写错的在界面上长得一模一样（补 32 的取舍）
        + '这一项没有"让模型补"的按钮——原文没写就只能你来定，模型猜出来的会当成正史。',
      card_ids: [c.id].filter(Boolean),
      // 出口：落到原著页的时间线卡上，就地手改 when（补 34 之前这一项只有结论、没有出口）
      go: {
        page: 'novel',
